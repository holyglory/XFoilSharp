...
public static class ViscousSolverEngine
{
    ...
    private const double LegacyLaminarShearSeed = 0.03d;
    ...
    private static double GetHvRat(bool useLegacyPrecision)
    {
        ...
        // in the live BL path; the double tree always uses the legacy value so
        ...
        for (int iter = 0; iter < maxIter; iter++)
        {
            ...
            if (!double.IsInfinity(rmsbl) && !double.IsNaN(rmsbl))
            {
                ...
                if (settings.UseLegacyBoundaryLayerInitialization)
                {
                    ...
                    legacyIncrementalCl = (double)((double)legacyIncrementalCl + (double)((double)newtonRlx * (double)dac));
                }
    ...
    {
        // Phase 1 strip: double-only COMSET. REAL throughout, including
        // GAMM1 = GAMMA - 1.0. The doubled tree (auto-generated *.Double.cs
        // twin via gen-double.py) replaces these floats with doubles.
        double machf = (double)mach;
        double qinff = (double)qinf;
        double reinff = (double)reinf;
        double hvratf = (double)hvrat;
        double gm1f = (double)LegacyPrecisionMath.GammaMinusOne(false);

        qinfbl = qinff;

        double msqf = machf * machf;
        double betaf = Math.Sqrt(1.0d - msqf);
        double betaMsqf = -0.5d / betaf;
        double betaOnef = 1.0d + betaf;
        tkbl = msqf / (betaOnef * betaOnef);
        tkbl_ms = (1.0d / (betaOnef * betaOnef))
            - ((2.0d * (double)tkbl / betaOnef) * betaMsqf);

        double gm1hf = 0.5d * gm1f;
        double denf = 1.0d + (gm1hf * msqf);
        rstbl = Math.Pow(denf, 1.0d / gm1f);
        rstbl_ms = 0.5d * (double)rstbl / denf;
        ...
        double qinfSqInvf = Math.Pow(1.0d / qinff, 2.0d);
        hstinv = gm1f * Math.Pow(machf / qinff, 2.0d) / denf;
        hstinv_ms = (gm1f * qinfSqInvf / denf)
            - ((0.5d * gm1f * (double)hstinv) / denf);

        double heratf = 1.0d - (0.5d * qinff * qinff * (double)hstinv);
        double heratMsf = -0.5d * qinff * qinff * (double)hstinv_ms;
        double reyScalef = Math.Sqrt(heratf * heratf * heratf) * (1.0d + hvratf) / (heratf + hvratf);
        reybl = reinff * reyScalef;
        reybl_re = reyScalef;
        reybl_ms = (double)reybl * ((1.5d / heratf) - (1.0d / (heratf + hvratf))) * heratMsf;
    }
    ...
    {
        // Phase 1 strip: double-only path always rounds to single.
        uei = LegacyPrecisionMath.RoundToSingle(uei, false);
        theta = LegacyPrecisionMath.RoundToSingle(theta, false);
        dstar = LegacyPrecisionMath.RoundToSingle(dstar, false);
        shearLikeState = LegacyPrecisionMath.RoundToSingle(shearLikeState, false);
    }
        ...
        out double ctauTe)
    {
        ...
        thetaTe = useLegacyPrecision
            ? LegacyPrecisionMath.Add(upperThetaTe, lowerThetaTe, false)
        ...
        double[]? overrideQvis = null)
    {
        ...
        // Phase 1 strip: double-only Fortran CLCALC with REAL arithmetic
        ...
        double fCa = LegacyPrecisionMath.CosD((double)alphaRadians);
        double fSa = LegacyPrecisionMath.SinD((double)alphaRadians);
        double fQinf = (double)Math.Max(qinf, 1e-10);
        double fCl = 0.0d;

        // Fortran CLCALC: initialize CPG1 at node 1 (index 0).
        double q1 = (double)qvis[0];
        double cginc1 = 1.0d - (q1 / fQinf) * (q1 / fQinf);
        double cpg1 = cginc1; // For M=0: BETA=1, BFAC=0 -> CPG = CGINC
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            double qip = (double)qvis[ip];
            double cginc2 = 1.0d - (qip / fQinf) * (qip / fQinf);
            double cpg2 = cginc2;
            ...
            double dxTerm = LegacyPrecisionMath.RoundBarrier(
                ((double)panel.X[ip] - (double)panel.X[i]) * fCa);
            double dyTerm = LegacyPrecisionMath.RoundBarrier(
                ((double)panel.Y[ip] - (double)panel.Y[i]) * fSa);
            double dx = LegacyPrecisionMath.RoundBarrier(dxTerm + dyTerm);
            double ag = LegacyPrecisionMath.RoundBarrier(0.5d * (cpg2 + cpg1));

            // Fortran: CL = CL + DX*AG (separate multiply then add).
            double dxAg = LegacyPrecisionMath.RoundBarrier(dx * ag);
        ...
        for (int side = 0; side < 2; side++)
        {
            for (int ibl = 1; ibl < blState.NBL[side] && ibl <= blState.IBLTE[side]; ibl++)
            {
                ...
                if (iPan >= 0 && iPan < n)
                {
                    ...
                    speeds[iPan] = (double)blState.VTI[ibl, side] * (double)blState.UEDG[ibl, side];
                }
            }
        }
        ...
        bool useLegacyPrecision)
    {
        ...
        double sst;
        {
            double gammaLeft = (double)qinv[ist];
            double gammaRight = (double)qinv[ist + 1];
            double panelArcLeft = (double)panel.ArcLength[ist];
            double panelArcRight = (double)panel.ArcLength[ist + 1];
            double dgam = gammaRight - gammaLeft;
            double ds = panelArcRight - panelArcLeft;
            ...
            if (sst <= panelArcLeft) sst = panelArcLeft + 1.0e-7d;
            if (sst >= panelArcRight) sst = panelArcRight - 1.0e-7d;
        }
        ...
        if (ist >= 0 && ist + 1 < n)
        {
            double dgamF = (double)qinv[ist + 1] - (double)qinv[ist];
            if (Math.Abs(dgamF) > 1e-30d)
            {
                sstGo = ((double)sst - (double)panel.ArcLength[ist + 1]) / dgamF;
                sstGp = ((double)panel.ArcLength[ist] - (double)sst) / dgamF;
            }
        }
        ...
        double angleOfAttackRadians)
    {
        ...
        try
        {
            ...
            if (useBasisDecomposition)
            {
                ...
                double cosA = LegacyPrecisionMath.Cos(angleOfAttackRadians, false);
                double sinA = LegacyPrecisionMath.Sin(angleOfAttackRadians, false);
        ...
        double freestreamSpeed)
    {
        ...
        bool Differs(int index)
            => BitConverter.DoubleToInt64Bits((double)inviscidState.VortexStrength[index]) !=
               BitConverter.DoubleToInt64Bits((double)currentGamma[index]);
        ...
        LinearVortexPanelState panel)
    {
        ...
        // Phase 1 strip: double-only path. The doubled tree (auto-generated
        ...
        if (panel.NodeCount >= 2)
        {
            ...
            double xp1 = (double)panel.XDerivative[0];
            double yp1 = (double)panel.YDerivative[0];
            double xpN = (double)panel.XDerivative[nLast];
            double ypN = (double)panel.YDerivative[nLast];
            double crosp = (xp1 * ypN - yp1 * xpN)
                        / Math.Sqrt((xp1 * xp1 + yp1 * yp1) * (xpN * xpN + ypN * ypN));
            double dwdx = crosp / Math.Sqrt(1d - crosp * crosp);
            const double clampF = 3d / 2.5d;
        ...
        // subtracting back is NOT a no-op — double-rounding the sum loses ~4-5
        ...
        double baseArcF = (double)panel.ArcLength[panelLastIdx] - (double)panel.LeadingEdgeArcLength;
        double accumArcF = baseArcF;
        double distanceF = 0d;
        ...
        for (int iw = 0; iw < wakeGeometry.Count; iw++)
        {
            if (iw > 0)
            {
                double dxF = (double)wakeGeometry.X[iw] - (double)wakeGeometry.X[iw - 1];
                double dyF = (double)wakeGeometry.Y[iw] - (double)wakeGeometry.Y[iw - 1];
                double dxssi = Math.Sqrt((dxF * dxF) + (dyF * dyF));
                accumArcF += dxssi;
        ...
        bool useLegacyPrecision = false)
    {
        ...
        {
            // Phase 1 strip: double-only Fortran XICALC DXSSI.
            //   DXSSI = SQRT((X(I)-X(I-1))**2 + (Y(I)-Y(I-1))**2)
            double xCurr = (double)wakeSeed.Geometry.X[wakeIndex];
            double xPrev = (double)wakeSeed.Geometry.X[wakeIndex - 1];
            double yCurr = (double)wakeSeed.Geometry.Y[wakeIndex];
            double yPrev = (double)wakeSeed.Geometry.Y[wakeIndex - 1];
            double dxf = xCurr - xPrev;
            double dyf = yCurr - yPrev;
            return Math.Sqrt(dxf * dxf + dyf * dyf);
        }
    ...
    private static double ComputeMassDefect(double dstar, double ue, bool useLegacyPrecision = false)
    {
        return useLegacyPrecision
            ? LegacyPrecisionMath.Multiply(dstar, ue, useLegacyPrecision: false)
            : dstar * ue;
    }
        ...
        WakeSeedData? wakeSeed)
    {
        ...
        for (int side = 0; side < 2; side++)
        {
            ...
            if (settings.UseLegacyBoundaryLayerInitialization)
            {
                ...
                double xsi0f = (double)xsi0;
                double ue0f = (double)ue0;
                double reinff = (double)reinf;
                double bulef = 1.0d;
                double uconf = LegacyPrecisionMath.DivideD(
                    ue0f,
                    LegacyPrecisionMath.PowD(xsi0f, bulef));
                double tsqf = LegacyPrecisionMath.MultiplyD(
                    LegacyPrecisionMath.DivideD(
                        0.45d,
                        LegacyPrecisionMath.MultiplyD(
                            LegacyPrecisionMath.MultiplyD(
                                uconf,
                                LegacyPrecisionMath.AddD(
                                    LegacyPrecisionMath.MultiplyD(5.0d, bulef),
                                    1.0d)),
                            reinff)),
                    LegacyPrecisionMath.PowD(
                        xsi0f,
                        LegacyPrecisionMath.SubtractD(1.0d, bulef)));
                tsqf = Math.Max(tsqf, 1.0e-20d);
                double thif = Math.Sqrt(tsqf);
                double dsif = LegacyPrecisionMath.MultiplyD(2.2d, thif);
                thi = thif;
                dsi = dsif;
            ...
            blState.MASS[1, side] = settings.UseLegacyBoundaryLayerInitialization
                ? LegacyPrecisionMath.Multiply(dsi, blState.UEDG[1, side], useLegacyPrecision: false)
        ...
        WakeSeedData? wakeSeed)
    {
        ...
        // All REAL (double) ops. Compute each step in double to avoid a double
        ...
        double ctau = LegacyPrecisionMath.Divide(
            LegacyPrecisionMath.Add(
                LegacyPrecisionMath.Multiply(ctauUpper, upperThetaTe, false),
                LegacyPrecisionMath.Multiply(ctauLower, lowerThetaTe, false), false),
            theta, false);
        ctau = Math.Max(ctau, 0.03);
        theta = LegacyPrecisionMath.RoundToSingle(theta, false);
        dstar = LegacyPrecisionMath.RoundToSingle(dstar, false);
        ctau = LegacyPrecisionMath.RoundToSingle(ctau, false);
        ...
        double reybl_ms)
    {
        ...
        for (int ibl = startStation; ibl < blState.NBL[side]; ibl++)
        {
            ...
            double lastDmaxForExtrapolation = 0.0d;
            for (int iter = 0; iter < maxIterations; iter++)
            {
                ...
                lastDmaxForExtrapolation = (double)dmax;
                ...
                if (directMode && ibl != blState.IBLTE[side] + 1)
                {
                    ...
                    if (hkTest >= turbulentHkLimit)
                    {
                        ...
                        inverseTargetHk = settings.UseLegacyBoundaryLayerInitialization
                            ? (wake ? Math.Max((double)inverseTargetHk, 1.01d)
                                    : Math.Max((double)inverseTargetHk, (double)turbulentHkLimit))
                ...
                if ((double)dmax <= (double)seedTolerance)
                {
                    break;
                }
            }
            ...
            if (settings.UseLegacyBoundaryLayerInitialization)
            {
                double dmaxEndF = lastDmaxForExtrapolation;
                if (dmaxEndF > 0.1d && ibl > 2)
                {
                    if (ibl <= blState.IBLTE[side])
                    {
                        ...
                        double xRatio = (double)blState.XSSI[ibl, side] / (double)blState.XSSI[ibm, side];
                        double sqrtRatio = Math.Pow(xRatio, 0.5d);
                        theta = (double)blState.THET[ibm, side] * sqrtRatio;
                        dstar = (double)blState.DSTR[ibm, side] * sqrtRatio;
                    ...
                    else
                    {
                        ...
                        double thetaPrevF = (double)blState.THET[ibm, side];
                        double dstarPrevF = (double)blState.DSTR[ibm, side];
                        double xsiF = (double)blState.XSSI[ibl, side];
                        double xsiPrevF = (double)blState.XSSI[ibm, side];
                        double ratlen = (xsiF - xsiPrevF) / (10.0d * dstarPrevF);
                        theta = thetaPrevF;
                        dstar = (dstarPrevF + thetaPrevF * ratlen) / (1.0d + ratlen);
                    }
                    ...
                    {
                        double ueiF2 = (double)blState.UEDG[ibl, side];
                        if (ibl > 1 && ibl < blState.NBL[side] - 1)
                        {
                            double ueim = (double)blState.UEDG[ibm, side];
                            double ueip = (double)blState.UEDG[ibl + 1, side];
                            ueiF2 = 0.5d * (ueim + ueip);
                        }
                        uei = ueiF2;
                    }
            ...
            if (settings.UseLegacyBoundaryLayerInitialization)
            {
                ...
                {
                    primary.PreUpdateT = LegacyPrecisionMath.RoundToSingle(mrchuePrevT, false);
                    primary.PreUpdateD = LegacyPrecisionMath.Subtract(
                        mrchuePrevD, wakeGap, false);
                    primary.PreUpdateDFull = LegacyPrecisionMath.RoundToSingle(mrchuePrevD, false);
                }
        ...
        ref double carriedCtauOut)
    {
        ...
        for (int iter = 0; iter < maxIterations; iter++)
        {
            ...
            if (directMode)
            {
                ...
                double hmax = useLegacySeedPrecision
                    ? (usesShearState ? (double)legacyTransitionHkLimit : (double)legacyLaminarHkLimit)
        ...
        if (settings.UseLegacyBoundaryLayerInitialization)
        {
            double dmaxF = lastDmaxFloat;
            
            if (dmaxF > 0.1d && ibl > 2)
            {
                // Fortran xbl.f:2298-2299:
                //   THI = THET(IBM,IS) * (XSSI(IBL,IS)/XSSI(IBM,IS))**0.5
                // Gfortran compiles REAL**0.5 via powf(x, 0.5d), NOT sqrtf(x).
                // powf differs from sqrtf by 1 ULP at some x values (e.g.
                // x=1.0125 → sqrtf=3F80CC96 vs powf=3F80CC97). Using Math.Pow
                // matches Fortran's **0.5 bit-for-bit.
                double xRatio = (double)blState.XSSI[ibl, side] / (double)blState.XSSI[ibm, side];
                double sqrtRatio = Math.Pow(xRatio, 0.5d);
                theta2 = (double)blState.THET[ibm, side] * sqrtRatio;
                dstar2 = (double)blState.DSTR[ibm, side] * sqrtRatio;
                ...
                double ueiF = (double)blState.UEDG[ibl, side];
                if (ibl > 1 && ibl < blState.NBL[side] - 1)
                {
                    double ueim = (double)blState.UEDG[ibm, side];
                    double ueip = (double)blState.UEDG[ibl + 1, side];
        ...
        bool convergedInLoop = (double)lastDmaxFloat <= (double)seedTolerance;
        if (settings.UseLegacyBoundaryLayerInitialization
            && !convergedInLoop
            ...
            && ibl > 0) // not similarity station (Fortran: .NOT.SIMI)
        {
            ...
            // double-rounded values that will be stored in blState.THET/DSTR/UEDG.
            ApplyLegacySeedPrecision(
                settings.UseLegacyBoundaryLayerInitialization,
                ref uei2, ref theta2, ref dstar2, ref ampl2);
            ctau2 = LegacyPrecisionMath.RoundToSingle(ctau2, false);
        ...
        blState.MASS[ibl, side] = settings.UseLegacyBoundaryLayerInitialization
            ? LegacyPrecisionMath.Multiply(dstar2, uei2, useLegacyPrecision: false)
            : dstar2 * uei2;
        if (transitionInterval)
        {
            blState.ITRAN[side] = ibl;
        ...
        if (settings.UseLegacyBoundaryLayerInitialization)
        {
            ...
            if (primary is not null)
            {
                primary.PreUpdateT = LegacyPrecisionMath.RoundToSingle(preUpdateTheta2, false);
                primary.PreUpdateD = LegacyPrecisionMath.RoundToSingle(preUpdateDstar2, false);
            }
        }
        ...
        double? hkPrevOverride = null)
    {
        ...
        // Phase 1 strip: double-only Fortran MRCHUE backward-Euler.
        double hkPrevF = (double)hkPrev;
        double x1F = (double)x1;
        double x2F = (double)x2;
        double t1F = (double)prevTheta;
        double constF = 0.03d * (x2F - x1F) / t1F;
        double hk2F = hkPrevF;
        for (int iter = 0; iter < 3; iter++)
        {
            double hkm1 = hk2F - 1.0d;
            double hkm1sq = hkm1 * hkm1;
            double hkm1cu = hkm1sq * hkm1;
            double denom = 1.0d + 3.0d * constF * hkm1sq;
        ...
        double reybl_ms)
    {
        ...
        blState.MASS[ibl, side] = settings.UseLegacyBoundaryLayerInitialization
            ? LegacyPrecisionMath.Multiply(dstar, uei, useLegacyPrecision: false)
            : dstar * uei;
        ...
        double teGap)
    {
        ...
        blState.CTAU[ibl, side] = ctau;
        blState.MASS[ibl, side] = settings.UseLegacyBoundaryLayerInitialization
            ? LegacyPrecisionMath.Multiply(dstar, uei, false)
            : dstar * uei;
    }
        ...
        for (int side = 0; side < 2; side++)
        {
            ...
            for (int ibl = 1; ibl < blState.NBL[side]; ibl++)
            {
                ...
                double wakeGap = wake
                    ? GetWakeGap(wakeSeed, teGap, ibl - blState.IBLTE[side])
                    : 0.0;
                // Fortran: 1.02000 and 1.00005 are REAL constants → double multiply
                double hklim = wake ? 1.00005 : 1.02;
                if (settings.UseLegacyBoundaryLayerInitialization)
                {
                    double hklimF = wake ? 1.00005d : 1.02d;
                    double dswF = (double)wakeGap;
                    dstar = (double)Math.Max((double)dstar - dswF, hklimF * (double)theta) + dswF;
                }
                else
                {
                    dstar = Math.Max(dstar - wakeGap, hklim * theta) + wakeGap;
                ...
                for (int iter = 0; iter < 25; iter++)
                {
                    ...
                    else
                    {
                        ...
                        // solve bit-exact with Fortran's double arithmetic; the previous
                    ...
                    if (settings.UseLegacyBoundaryLayerInitialization)
                    {
                        // Fortran MRCHDU DMAX: all REAL (double) operations
                        double fd1 = (double)delta[1], fd2 = (double)delta[2], fd3 = (double)delta[3];
                        double fth = (double)theta, fds = (double)dstar, fue = (double)uei;
                        dmax = Math.Max(Math.Abs(fd1 / fth), Math.Abs(fd2 / fds));
                        dmax = Math.Max((double)dmax, Math.Abs(fd3 / fue));
                        if (ibl >= blState.ITRAN[side])
                        {
                            double fd0 = (double)delta[0], fct = (double)ctau;
                            dmax = Math.Max((double)dmax, Math.Abs(fd0 / (10.0d * fct)));
                        }
                    }
                    else
                    {
                    ...
                    // Fortran: IF(DMAX.LE.DEPS) — both REAL (double) comparison
                    lastDmaxForExtrapolation = (double)dmax;
                    
                    if ((double)dmax <= (double)legacySeedTolerance)
                    {
                        converged = true;
                        break;
                    }
                ...
                {
                    double dmaxF = lastDmaxForExtrapolation;
                    
                    if (dmaxF > 0.1d && ibl > 2)  // Fortran IBL>3 → C# ibl>2
                    {
                        if (ibl <= blState.IBLTE[side])
                        {
                            ...
                            // through libm powf in -O0; matching with Math.Sqrt
                            // can drift one ULP, so route through the same powf
                            // bridge that other parity-sensitive paths use.
                            double xRatioF = (double)blState.XSSI[ibl, side] / (double)blState.XSSI[ibm, side];
                            double sqrtRatioF = Math.Pow(xRatioF, 0.5d);
                            theta = (double)blState.THET[ibm, side] * sqrtRatioF;
                            dstar = (double)blState.DSTR[ibm, side] * sqrtRatioF;
                            uei = blState.UEDG[ibm, side];
                        ...
                        else if (ibl == blState.IBLTE[side] + 1)
                        {
                            ...
                            // Fortran: all REAL (double) additions
                            double tte = (double)blState.THET[blState.IBLTE[0], 0] + (double)blState.THET[blState.IBLTE[1], 1];
                            double dte = (double)blState.DSTR[blState.IBLTE[0], 0] + (double)blState.DSTR[blState.IBLTE[1], 1]
                                        + (double)GetWakeGap(wakeSeed, teGap, 0);
                            theta = tte;
                            dstar = dte;
                            uei = blState.UEDG[ibm, side];
                        }
                        else
                        {
                            ...
                            theta = blState.THET[ibm, side];
                            double ratlenF = ((double)blState.XSSI[ibl, side] - (double)blState.XSSI[ibm, side])
                                          / (10.0d * (double)blState.DSTR[ibm, side]);
                            dstar = ((double)blState.DSTR[ibm, side] + (double)theta * ratlenF) / (1.0d + ratlenF);
                            uei = blState.UEDG[ibm, side];
                        }
            ...
            for (int iter = 0; iter < maxIterations; iter++)
            {
                ...
                if (directMode && !firstWake)
                {
                    ...
                    if (hkTest >= wakeHkLimit)
                    {
                        ...
                        inverseTargetHk = settings.UseLegacyBoundaryLayerInitialization
                            ? Math.Max((double)inverseTargetHk, 1.01d)
                ...
                // Must use double comparison to match exactly, not double.
                if ((double)dmax <= (double)legacySeedTolerance)
                {
                    break;
                }
    ...
    private static PrimaryStationState? ResolveLegacyPrimaryStationStateOverride(
        ...
        bool useLegacyPrecision)
    {
        // Phase 1 strip: double-only path always returns the legacy primary.
    ...
    private static PrimaryStationState? CreateLegacyPrimaryStationStateOverride(
        ...
        bool useLegacyPrecision = false)
    {
        // Phase 1 strip: double-only path always builds the legacy carry state.
        ...
        {
            U = carriedU,
            T = LegacyPrecisionMath.RoundToSingle(theta, false),
            D = LegacyPrecisionMath.RoundToSingle(dstar, false)
        };
    }
    ...
    // Difference from legacy: The solve is delegated to the shared dense solver, with a parity-only double cast path that reproduces the single-precision seed solves explicitly.
    // Decision: Keep the shared helper and preserve the double solve path for parity mode.
    ...
    {
        int n = rhs.Length;
        // Phase 1 strip: double-only path follows classic XFoil's single-precision solves.
        ...
        var singleMatrix = SolverBuffers.DenseScratchMatrixDouble(rows, cols);
        var singleRhs = SolverBuffers.DenseScratchVectorDouble(n);
        for (int row = 0; row < rows; row++)
        {
            singleRhs[row] = (double)rhs[row];
            for (int col = 0; col < cols; col++)
            {
                singleMatrix[row, col] = (double)matrix[row, col];
            }
        }

        solver.SolveInPlace(singleMatrix, singleRhs);

        // Copy the double solution back into the caller's rhs buffer; the
        ...
        bool includeShearInDmax = true)
    {
        ...
        // seed stations, so keep the direct double recurrence here.
        double delta0f = (double)delta[0];
        double delta1f = (double)delta[1];
        double delta2f = (double)delta[2];
        double square0 = delta0f * delta0f;
        double square1 = delta1f * delta1f;
        double square2 = delta2f * delta2f;
        double wideSumSquares = (double)square0;
        wideSumSquares += (double)square1;
        wideSumSquares += (double)square2;
        double sumSquares = square0 + square1;
        sumSquares += square2;
        double residualNorm = Math.Sqrt(sumSquares);
    ...
    {
        // Phase 1 strip: double-only relaxation limiter.
        double dmaxf = (double)dmax;
        return dmaxf > 0.3d ? 0.3d / dmaxf : 1.0d;
    }
        ...
        LinearVortexPanelState panel)
    {
        ...
        if (n >= 2)
        {
            ...
            // Fortran computes CROSP/DWDXTE in REAL (double) arithmetic
            double xp1f = (double)panel.XDerivative[0];
            double yp1f = (double)panel.YDerivative[0];
            double xpNf = (double)panel.XDerivative[n - 1];
            double ypNf = (double)panel.YDerivative[n - 1];
            double denomSq = (xp1f * xp1f + yp1f * yp1f) * (xpNf * xpNf + ypNf * ypNf);
            ...
            if (denom > 1e-20d)
            {
                ...
                double crosp = (xp1f * ypNf - yp1f * xpNf) / denom;
                dwdxte = crosp / Math.Sqrt(1.0d - crosp * crosp);
            }
        }

        
        dwdxte = Math.Max(dwdxte, -3.0d / telrat);
        dwdxte = Math.Min(dwdxte, 3.0d / telrat);

        

        double aa = 3.0d + telrat * dwdxte;
        double bb = -2.0d - telrat * dwdxte;
        double antef = (double)ante;
        ...
        for (int iw = 0; iw < count; iw++)
        {
            ...
            double xssiDiff = (double)(blState.XSSI[ibl, side] - xssiTe);
            double zn = 1.0d - xssiDiff / (telrat * antef);
            ...
            // different double result than `* (zn*zn)`.
            double znSq = zn * zn;
            double wgapVal = (zn >= 0.0d) ? antef * (aa + bb * zn) * znSq : 0.0d;
