...
public static class ViscousNewtonUpdater
{
        ...
        bool useLegacyPrecision = false)
    {
        ...
        if (mode == ViscousSolverMode.XFoilRelaxation)
        {
            ...
            // Phase 1 strip: double-only path uses the in-loop normalized RMS.
        ...
        bool useLegacyPrecision)
    {
        ...
        {
            // Phase 1 strip: double-only DCL clamp.
            double fDclMax = 0.5d;
            double fDclMin = -0.5d;
            double fRlxDac = (double)rlx * (double)coupling.Dac;
            if (fRlxDac > fDclMax)
                rlx = fDclMax / (double)coupling.Dac;
            if (fRlxDac < fDclMin)
                rlx = fDclMin / (double)coupling.Dac;
        }
        ...
        for (int jv = 0; jv < nsys; jv++)
        {
            ...
            // Phase 1 strip: double-only RMSBL accumulation. Fortran:
            //   RMSBL = RMSBL + DN1**2 + DN2**2 + DN3**2 + DN4**2 — REAL,
            //   left-to-right with each term added separately (NOT pre-summed).
            double fdn1 = (double)dn1, fdn2 = (double)dn2, fdn3 = (double)dn3, fdn4 = (double)dn4;
            double fAcc = (double)rmsAccum;
            fAcc = fAcc + fdn1 * fdn1;
            fAcc = fAcc + fdn2 * fdn2;
            fAcc = fAcc + fdn3 * fdn3;
            fAcc = fAcc + fdn4 * fdn4;
        ...
        // Phase 1 strip: double-only Fortran RMSBL = SQRT( RMSBL / (4.0*FLOAT(NBL(1)+NBL(2))) )
        double fRms = (double)rmsAccum;
        double fDenom = 4.0d * (double)totalStations;
        double normalizedRmsbl = Math.Sqrt(fRms / fDenom);
            ...
            for (int jv = 0; jv < nsys; jv++)
            {
                ...
                if (iPan >= 0 && jPan >= 0 && iPan < dij.GetLength(0) && jPan < dij.GetLength(1))
                {
                    ueSum = LegacyPrecisionMath.Add(
                        ueSum,
                        -LegacyPrecisionMath.Multiply(vtiI, vtiJ, dij[iPan, jPan], useLegacyPrecision) * (double)dMass,
                        useLegacyPrecision);
                }
            }
        ...
        bool useLegacyPrecision)
    {
        ...
        for (int iv = 0; iv < nsys; iv++)
        {
            ...
            for (int jv = 0; jv < nsys; jv++)
            {
                ...
                // Phase 1 strip: double-only Fortran UPDATE MASS + VDEL in REAL.
                double dMassValue = (double)((double)blState.MASS[jbl, jside] + (double)vdel[2, 0, jv]);
                double dMassControl = (double)(-(double)vdel[2, 1, jv]);
            ...
            // Phase 1 strip: double-only path. In the alpha-prescribed legacy
            // UPDATE path UINV_AC is zero, but the stored control velocity
            // still passes through REAL assignment rounding.
            uNew[ibl, iside] = (double)((double)context.UeInv[ibl, iside] + (double)ueSum);
            uAc[ibl, iside] = (double)ueControl;
        ...
        double clNew = 0.0;
        double clAc = 0.0;
        // Fortran URELAX uses `SA = SIN(ALFA); CA = COS(ALFA)` in REAL (double).
        ...
        // helper so parity mode lowers to double sinf/cosf matching Fortran.
        // Fortran URELAX uses `SA = SIN(ALFA); CA = COS(ALFA)` in REAL (double).
        // Route through legacy-aware helper so parity mode lowers to double sinf/cosf.
        ...
        // Phase 1 strip: double-only Fortran URELAX CL integration.
        {
            double fClNew = 0.0d;
            double fClAc = 0.0d;
            double fCa = (double)cosa;
            double fSa = (double)sina;
            double fQinf = (double)qinfSafe;
            double fQinf2 = fQinf * fQinf;

            // Fortran: initialize CPG1 at node 1.
            double q1 = (double)qNew[0];
            double cginc1 = 1.0d - (q1 / fQinf) * (q1 / fQinf);
            double cpg1 = cginc1; // For M=0: BETA=1, BFAC=0 -> CPG = CGINC
            double cpiQ1 = -2.0d * q1 / fQinf2;
            double cpg1Ac = cpiQ1 * (double)qAc[0];

            for (int i = 0; i < n; i++)
            {
                int ip = (i + 1 == n) ? 0 : i + 1;

                double qip = (double)qNew[ip];
                double cginc2 = 1.0d - (qip / fQinf) * (qip / fQinf);
                double cpg2 = cginc2;
                double cpiQ2 = -2.0d * qip / fQinf2;
                double cpg2Ac = cpiQ2 * (double)qAc[ip];

                double dxPhys = (double)context.Panel.X[ip] - (double)context.Panel.X[i];
                double dyPhys = (double)context.Panel.Y[ip] - (double)context.Panel.Y[i];
                double dx = dxPhys * fCa + dyPhys * fSa;

                double ag = 0.5d * (cpg2 + cpg1);
                double agAc = 0.5d * (cpg2Ac + cpg1Ac);
        ...
        // Phase 1 strip: double-only Fortran DAC = (CLNEW - CL) / (1.0 - CL_AC) — all REAL.
        double fDacDenom = 1.0d - (double)clAc;
        double dac = Math.Abs(fDacDenom) > 1e-30d
            ? ((double)clNew - (double)context.CurrentCl) / fDacDenom
            : 0.0d;
        ...
        for (int jv = 0; jv < nsys; jv++)
        {
            ...
            // All REAL operations with per-op rounding for double parity.
