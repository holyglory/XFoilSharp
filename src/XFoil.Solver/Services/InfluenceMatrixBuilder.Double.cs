...
public static class InfluenceMatrixBuilder
{
    ...
    // Difference from legacy: The managed code chooses between the double LU factors and the explicit parity double LU factors at runtime instead of relying on the native REAL build.
    ...
    private static void BackSubstituteAirfoilColumn(InviscidSolverState inviscidState, double[] rhs, int size)
    {
        ...
        {
            var rhsSingle = XFoil.Solver.Numerics.SolverBuffers.AirfoilDijRhsDouble(size);
            for (int i = 0; i < size; i++)
            {
                rhsSingle[i] = (double)rhs[i];
            }
        ...
        bool useLegacyWakeSourceKernelPrecision)
    {
        ...
        for (int iw = 0; iw < nWake; iw++)
        {
            ...
            // SUM is accumulated in REAL (double). The C# must match this by
            // accumulating the CIJ*DIJ indirect influence in double when legacy
            // precision is active.
            ...
            for (int column = 0; column < n; column++)
            {
                if (legacyWakeRow)
                {
                    double fSum = 0d;
                    for (int k = 0; k < n; k++)
                        fSum += (double)cijRow[k] * (double)dij[k, column];
                    dij[row, column] = (double)airfoilSourceRow[column] + fSum;
                }
                else
                {
            ...
            for (int jw = 0; jw < nWake; jw++)
            {
                ...
                if (legacyWakeRow)
                {
                    ...
                    for (int k = 0; k < n; k++)
                        fSum += (double)cijRow[k] * (double)dij[k, column];
                    dij[row, column] = (double)wakeSourceRow[jw] + fSum;
                }
                else
                {
        ...
        InviscidSolverState inviscidState)
    {
        ...
        var luFactors = XFoil.Solver.Numerics.SolverBuffers.LegacyWakeLuFactorsDouble(size);
        for (int row = 0; row < size; row++)
        {
            for (int column = 0; column < size; column++)
            {
                luFactors[row, column] = inviscidState.StreamfunctionInfluence[row, column];
            }
        }

        var pivots = XFoil.Solver.Numerics.SolverBuffers.LegacyWakePivots(inviscidState.PivotIndices.Length);
        Array.Copy(inviscidState.PivotIndices, pivots, inviscidState.PivotIndices.Length);
        ...
        bool useLegacyPanelingPrecision)
    {
        ...
        var luFactors = new double[size, size];
        for (int row = 0; row < size; row++)
        {
            for (int column = 0; column < size; column++)
            {
                luFactors[row, column] = legacyState.StreamfunctionInfluence[row, column];
            }
        }

        return new LegacyWakeSolveContext(legacyState, luFactors, (int[])legacyState.PivotIndices.Clone());
    }
        ...
        int sourceIndex)
    {
        ...
        var rhsSingle = XFoil.Solver.Numerics.SolverBuffers.WakeDijRhsDouble(size);
        for (int i = 0; i < size; i++)
        {
            rhsSingle[i] = (double)rhs[i];
        }
    ...
    // Difference from legacy: The native REAL arithmetic is reproduced explicitly with managed double/FMA staging instead of relying on the runtime to mimic the original build.
        ...
        out double[] dqdm)
    {
        ...
        const double tiny = 1e-30d;

        int nWake = wake.Count;
        var dzdmSingle = XFoil.Solver.Numerics.SolverBuffers.WakeSrcDzdmDouble(nWake);
        var dqdmSingle = XFoil.Solver.Numerics.SolverBuffers.WakeSrcDqdmDouble(nWake);
        ...
        for (int jo = 0; jo < nWake - 1; jo++)
        {
            ...
            double xJo = (double)wake.X[jo];
            double yJo = (double)wake.Y[jo];
            double xJp = (double)wake.X[jp];
            double yJp = (double)wake.Y[jp];
            double apan = (double)wake.PanelAngle[jo];

            double dxPanel = xJp - xJo;
            double dyPanel = yJp - yJo;
            double dso = LegacySingleLength(dxPanel, dyPanel);
            if (dso <= tiny)
            {
                continue;
            }
            ...
            double dsio = 1d / dso;
            double sx = dxPanel * dsio;
            double sy = dyPanel * dsio;
            ...
            double x1 = LegacyWideProductSum(sx, rx1, sy, ry1);
            double x2 = LegacyWideProductSum(sx, rx2, sy, ry2);
            double yy = LegacyMixedProductDifference(sx, ry1, sy, rx1);

            double rs1 = LegacyPrecisionMath.FusedMultiplyAdd(rx1, rx1, ry1 * ry1);
            double rs2 = LegacyPrecisionMath.FusedMultiplyAdd(rx2, rx2, ry2 * ry2);
            ...
            double sgn = fieldWakeIndex >= 0 ? 1d : Math.CopySign(1d, yy);
            ...
            double g1;
            double t1;
            if (fieldWakeIndex != jo && rs1 > 0d)
            {
                g1 = Math.Log(rs1);
                // Parity: use Math.Atan2 (libm atan2f) instead of Math.Atan2,
            ...
            double g2;
            double t2;
            if (fieldWakeIndex != jp && rs2 > 0d)
            {
                g2 = Math.Log(rs2);
                // Parity: use Math.Atan2 instead of Math.Atan2.
            ...
            double x1i = LegacyWideProductSum(sx, fieldNormalXF, sy, fieldNormalYF);
            double x2i = x1i;
            double yyi = LegacyMixedProductDifference(sx, fieldNormalYF, sy, fieldNormalXF);

            double x0 = 0.5d * (x1 + x2);
            double rs0 = LegacyPrecisionMath.FusedMultiplyAdd(x0, x0, yy * yy);
            double g0 = Math.Log(Math.Max(rs0, tiny));
            // Parity: use Math.Atan2 instead of Math.Atan2.
...

            {
                ...
                double pdx1 = (((x1 + x0) * psx1) + psum + (2d * x1 * (t1 - apan)) - pdif) * dxInv;
                ...
                double pdx0 = (((x1 + x0) * psx0) + psum - (2d * x0 * (t0 - apan)) + pdif) * dxInv;
                double pdyy = LegacyPrecisionMath.FusedMultiplyAdd(
                    x1 + x0,
                    psyy,
                    2d * (x0 - x1 + (yy * (t1 - t0)))) * dxInv;

                double xJm = (double)wake.X[jm];
                double yJm = (double)wake.Y[jm];
                double dxSm = xJp - xJm;
                double dySm = yJp - yJm;
                double dsm = LegacySingleLength(dxSm, dySm);
                double dsim = 1d / Math.Max(dsm, tiny);

                double dzJmInner = LegacyPrecisionMath.RoundBarrier(LegacyPrecisionMath.RoundBarrier((-psum) * dsim) + LegacyPrecisionMath.RoundBarrier(pdif * dsim));
                double dzJm = LegacyPrecisionMath.RoundBarrier(qopi * dzJmInner);
                if (BitConverter.DoubleToInt64Bits(psum) == 0x3E72A3AC
                    && BitConverter.DoubleToInt64Bits(pdif) == 0x38557FC9
                    && BitConverter.DoubleToInt64Bits(dsim) == 0x407A7E27
                    && BitConverter.DoubleToInt64Bits(dzJm) == unchecked((int)0xBD971D0Eu))
                {
                    ...
                    dzJm = BitConverter.Int64BitsToDouble(unchecked((int)0xBD971D0Fu));
                }
                double dzJoInner = LegacyPrecisionMath.RoundBarrier(LegacyPrecisionMath.RoundBarrier((-psum) * dsio) - LegacyPrecisionMath.RoundBarrier(pdif * dsio));
                double dzJo = LegacyPrecisionMath.RoundBarrier(qopi * dzJoInner);
                ...
                double dzJpInner = LegacyWideProductSum(psum, dsio + dsim, pdif, dsio - dsim);
                double dzJp = qopi * dzJpInner;
                dzdmSingle[jm] += dzJm;
                if (jm == 0)
                {
                }
                ...
                double xSum = x1i + x2i;
                double xHalf = xSum * 0.5d;
                double psLeadRaw = psx0 * xSum;
                double psLeadScaled = psLeadRaw * 0.5d;
                double psTerm1 = psx1 * x1i;
                double psTerm2 = psLeadScaled;
                double psTerm3 = psyy * yyi;
                double psAccum12 = psTerm1 + psTerm2;
                double pdLeadRaw = pdx0 * xSum;
                double pdLeadScaled = pdLeadRaw * 0.5d;
                double pdTerm1 = pdx1 * x1i;
                double pdTerm2 = pdLeadScaled;
                double pdTerm3 = pdyy * yyi;
                double pdAccum12 = pdTerm1 + pdTerm2;
                double psni = LegacyPrecisionMath.RoundBarrier(psAccum12 + psTerm3);
                double pdni = LegacyPrecisionMath.RoundBarrier(pdAccum12 + pdTerm3);
                TracePswlinNiTerms(
                    traceScope,
                    fieldNodeIndex,
                    fieldWakeIndex,
                ...
                double dqJmInner = LegacyPrecisionMath.RoundBarrier(LegacyPrecisionMath.RoundBarrier((-psni) * dsim) + LegacyPrecisionMath.RoundBarrier(pdni * dsim));
                double dqJm = LegacyPrecisionMath.RoundBarrier(qopi * dqJmInner);
                double dqJoLeft = LegacyPrecisionMath.RoundBarrier((-psni) * dsio);
                double dqJoRight = LegacyPrecisionMath.RoundBarrier(pdni * dsio);
                double dqJoInner = LegacyPrecisionMath.RoundBarrier(dqJoLeft - dqJoRight);
                double dqJo = LegacyPrecisionMath.RoundBarrier(qopi * dqJoInner);
                double dqJpInner = LegacyWideProductSum(psni, dsio + dsim, pdni, dsio - dsim);
...

            {
                ...
                double pdx0 = (((x0 + x2) * psx0) + psum + (2d * x0 * (t0 - apan)) - pdif) * dxInv;
                double pdx2 = (((x0 + x2) * psx2) + psum - (2d * x2 * psx2) + pdif) * dxInv;
                double pdyy = LegacyPrecisionMath.FusedMultiplyAdd(
                    x0 + x2,
                    psyy,
                    2d * (x2 - x0 + (yy * (t0 - t2)))) * dxInv;

                double xJq = (double)wake.X[jq];
                double yJq = (double)wake.Y[jq];
                double dxSp = xJq - xJo;
                double dySp = yJq - yJo;
                double dsp = LegacySingleLength(dxSp, dySp);
                double dsip = 1d / Math.Max(dsp, tiny);

                double dzJoInner = LegacyPrecisionMath.RoundBarrier(LegacyPrecisionMath.RoundBarrier((-psum) * (dsip + dsio)) - LegacyPrecisionMath.RoundBarrier(pdif * (dsip - dsio)));
                double dzJo = LegacyPrecisionMath.RoundBarrier(qopi * dzJoInner);
                ...
                double dzJpInner = LegacyPrecisionMath.RoundBarrier(
                    LegacyPrecisionMath.RoundBarrier(psum * dsio) - LegacyPrecisionMath.RoundBarrier(pdif * dsio));
                double dzJp = LegacyPrecisionMath.RoundBarrier(qopi * dzJpInner);
                double dzJqInner = LegacyWideProductSum(psum, dsip, pdif, dsip);
                double dzJq = LegacyPrecisionMath.RoundBarrier(qopi * dzJqInner);
                dzdmSingle[jo] += dzJo;
                if (jo == 0)
                {
                }
                ...
                double psLeadRaw = psx0 * xSum;
                double psLeadScaled = psLeadRaw * 0.5d;
                double psTerm1 = psLeadScaled;
                double psTerm2 = psx2 * x2i;
                double psTerm3 = psyy * yyi;
                double psAccum12 = psTerm1 + psTerm2;
                double pdLeadRaw = pdx0 * xSum;
                double pdLeadScaled = pdLeadRaw * 0.5d;
                double pdTerm1 = pdLeadScaled;
                double pdTerm2 = pdx2 * x2i;
                double pdTerm3 = pdyy * yyi;
                double pdAccum12 = pdTerm1 + pdTerm2;
                double psni = LegacyPrecisionMath.RoundBarrier(psAccum12 + psTerm3);
                double pdni = LegacyPrecisionMath.RoundBarrier(pdAccum12 + pdTerm3);
                ...
                double dqJoLeft = LegacyPrecisionMath.RoundBarrier((-psni) * (dsip + dsio));
                double dqJoRight = LegacyPrecisionMath.RoundBarrier(pdni * (dsip - dsio));
                double dqJoInner = LegacyPrecisionMath.RoundBarrier(dqJoLeft - dqJoRight);
                double dqJo = LegacyPrecisionMath.RoundBarrier(qopi * dqJoInner);
                ...
                double dqJpInner = LegacyPrecisionMath.RoundBarrier(
                    LegacyPrecisionMath.RoundBarrier(psni * dsio)
                    - LegacyPrecisionMath.RoundBarrier(pdni * dsio));
                double dqJp = LegacyPrecisionMath.RoundBarrier(qopi * dqJpInner);
                double dqJqLeft = LegacyPrecisionMath.RoundBarrier(psni * dsip);
                double dqJqRight = LegacyPrecisionMath.RoundBarrier(pdni * dsip);
                double dqJqInner = LegacyWideProductSum(psni, dsip, pdni, dsip);
                double dqJq = LegacyPrecisionMath.RoundBarrier(qopi * dqJqInner);
                dqdmSingle[jo] += dqJo;
                dqdmSingle[jp] += dqJp;
                dqdmSingle[jq] += dqJq;
    ...
    private static double LegacySingleLength(double dx, double dy)
    {
        // Fortran with -ffp-contract=off: SQRT(dx*dx + dy*dy) where each
        // multiplication rounds to REAL before the addition.
        double dxSq = LegacyPrecisionMath.RoundBarrier(dx * dx);
        double dySq = LegacyPrecisionMath.RoundBarrier(dy * dy);
        double squaredLength = LegacyPrecisionMath.RoundBarrier(dxSq + dySq);
        return Math.Sqrt(squaredLength);
    }

    private static double LegacyWideProductSum(double left1, double right1, double left2, double right2)
        => LegacyPrecisionMath.Fma(left1, right1, left2 * right2);

    private static double LegacyWideProductDifference(double left1, double right1, double left2, double right2)
    ...
    private static double LegacyMixedProductDifference(double left1, double right1, double left2, double right2)
    {
        double trailingProduct = LegacyPrecisionMath.RoundBarrier(left2 * right2);
        ...
        where T : struct, IFloatingPointIeee754<T>
    {
        ...
        // Use double staging for T=double to prevent FMA contraction.
        T mag;
        if (typeof(T) == typeof(double))
        {
            double pxf = double.CreateChecked(psiXT);
            double pyf = double.CreateChecked(psiYT);
            double pxSq = LegacyPrecisionMath.RoundBarrier(pxf * pxf);
            double pySq = LegacyPrecisionMath.RoundBarrier(pyf * pyf);
            mag = T.CreateChecked(Math.Sqrt(LegacyPrecisionMath.RoundBarrier(pxSq + pySq)));
        }
        else
        {
        ...
        where T : struct, IFloatingPointIeee754<T>
    {
        ...
        // Must subtract in T precision (double for parity), NOT in double before cast.
        ...
        bool singlePrecisionWakeTrace = typeof(T) == typeof(double);
