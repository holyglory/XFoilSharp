...
public static class PanelGeometryBuilder
{
    ...
    // Difference from legacy: The original routine relies on REAL arithmetic implicitly, while the managed port chooses between double and parity double execution explicitly.
    ...
    // Difference from legacy: The legacy routine always executes in REAL precision, while this overload chooses managed double or parity double execution explicitly.
        ...
        bool useLegacyPrecision)
    {
        ...
        // Math.Atan2 drifts 1-3 ULP from glibc atan2f at certain input
    ...
    private static void ComputePanelAnglesLegacyFloat(LinearVortexPanelState panel, InviscidSolverState state)
    {
        ...
        for (int i = 0; i < n - 1; i++)
        {
            double sx = (double)panel.X[i + 1] - (double)panel.X[i];
            double sy = (double)panel.Y[i + 1] - (double)panel.Y[i];

            if (sx == 0d && sy == 0d)
            {
                panel.PanelAngle[i] = Math.Atan2(-(double)panel.NormalY[i], -(double)panel.NormalX[i]);
            }
            else
            {
        ...
        {
            double sx = (double)panel.X[0] - (double)panel.X[last];
            double sy = (double)panel.Y[0] - (double)panel.Y[last];
    ...
    /// <summary>
    /// Legacy precision (double) overload of ContinuousAtan2 — uses libm atan2f
    ...
    {
        // Phase 1 strip: legacy double path. Uses libm atan2f to match Fortran's
        // REAL ATAN2 bit-for-bit. The doubled tree replaces atan2f with the
        // IEEE double Math.Atan2.
        const double twoPi = 2.0d * Math.PI;

        double newAngle = Math.Atan2((double)y, (double)x);
        double deltaTheta = newAngle - (double)referenceAngle;

        double correction = deltaTheta - twoPi * Math.Truncate((deltaTheta + Math.CopySign(Math.PI, deltaTheta)) / twoPi);

        return (double)referenceAngle + correction;
    }
}
