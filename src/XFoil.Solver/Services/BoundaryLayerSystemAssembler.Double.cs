...
public static class BoundaryLayerSystemAssembler
{
    ...
    private static readonly double LegacyCtcon = ComputeLegacyCtcon();
    ...
    private static double ComputeLegacyCtcon()
    {
        ...
        double gacon = (double)GACON;
        double gbcon = (double)GBCON;
        double gaconSq = gacon * gacon;
        return 0.5d / (gaconSq * gbcon);
    }
    ...
    {
        // Phase 1 strip: double-only BLPRV. Stores values in REAL slots; classic
        ...
        double ueif = (double)uei;
        double tkblf = (double)tkbl;
        double qinfblf = (double)qinfbl;
        double tkblMsf = (double)tkbl_ms;

        double ueiOq2f = (double)(((double)ueif / qinfblf) * ((double)ueif / qinfblf));
        double denomf = (double)(1.0 - ((double)tkblf * ueiOq2f));
        double u2f = (double)(((double)ueif * (1.0 - tkblf)) / denomf);
        double u2Ueif = (double)(
            (1.0 + ((double)tkblf * (((2.0 * u2f * ueif) / ((double)qinfblf * qinfblf)) - 1.0)))
            / denomf);
        double u2Msf = (double)(((((double)u2f * ueiOq2f) - ueif) * tkblMsf) / denomf);
        ...
        KinematicResult? destination = null)
    {
        ...
        //     field and then read it back via `(double)result.M2` on the next
        //     line — every read forced a heap load + narrow cast. Since the
        //     write is a lossless widening of the just-computed double into
        //     a double field, reading it back always produces the same double;
        //     replacing the read with the already-computed local is bit-exact
        //     equivalent.
        //   * Duplicated `Math.Pow(tr2f, -1/gm1blf)` / `Math.Pow(tr2,
        ...
        // Phase 1 strip: double-only BLKIN. The doubled tree (auto-generated
        // *.Double.cs twin via gen-double.py) gets the double-precision mirror.
        double u2f = (double)u2;
        double t2f = (double)t2;
        double d2f = (double)d2;
        // dw2 is not used in the legacy branch (preserves original behaviour).
        double hstinvf = (double)hstinv;
        double hstinvMsf = (double)hstinv_ms;
        double gm1blf = (double)gm1bl;
        double rstblf = (double)rstbl;
        double rstblMsf = (double)rstbl_ms;
        double hvratf = (double)hvrat;
        double reyblf = (double)reybl;
        double reyblRef = (double)reybl_re;
        double reyblMsf = (double)reybl_ms;

        // --- Mach number and derivatives ---
        double u2sqHstinv = u2f * u2f * hstinvf;
        double legacyM2Den = gm1blf * (1.0d - (0.5d * u2sqHstinv));
        double m2f = u2sqHstinv / legacyM2Den;
        double tr2f = 1.0d + (0.5d * gm1blf * m2f);
        double m2U2f = 2.0d * m2f * tr2f / u2f;
        double legacyM2MsNum = u2f * u2f * tr2f;
        double m2MsF = (legacyM2MsNum / legacyM2Den) * hstinvMsf;

        // --- Edge density (isentropic). Cache pow(tr2, -1/gm1): the original
        //     code called it twice with identical arguments. ---
        double trPowNeg1OverGm1 = Math.Pow(tr2f, -1.0d / gm1blf);
        double r2f = rstblf * trPowNeg1OverGm1;
        double r2U2f = -r2f / tr2f * 0.5d * m2U2f;
        double r2MsF = (-r2f / tr2f * 0.5d * m2MsF) + (rstblMsf * trPowNeg1OverGm1);

        // --- Shape parameter ---
        double h2f = d2f / t2f;
        double h2D2f = 1.0d / t2f;
        double h2T2f = -h2f / t2f;

        // --- Sutherland's viscosity law ---
        double legacyHerat = 1.0d - (0.5d * u2f * u2f * hstinvf);
        double legacyHeU2 = -u2f * hstinvf;
        double legacyHeMs = -(0.5d * u2f * u2f * hstinvMsf);

        double legacyV2 = Math.Sqrt(legacyHerat * legacyHerat * legacyHerat) * (1.0d + hvratf) / (legacyHerat + hvratf) / reyblf;
        double legacyV2He = legacyV2 * ((1.5d / legacyHerat) - (1.0d / (legacyHerat + hvratf)));
        double legacyV2U2 = LegacyPrecisionMath.MultiplyD(legacyV2He, legacyHeU2);
        double legacyV2OverRe = LegacyPrecisionMath.DivideD(legacyV2, reyblf);
        double legacyV2MsReyblTerm = LegacyPrecisionMath.MultiplyD(-legacyV2OverRe, reyblMsf);
        double legacyV2MsHeTerm = LegacyPrecisionMath.MultiplyD(legacyV2He, legacyHeMs);
        double legacyV2Ms = LegacyPrecisionMath.AddD(legacyV2MsReyblTerm, legacyV2MsHeTerm);
        double legacyV2Re = LegacyPrecisionMath.MultiplyD(-legacyV2OverRe, reyblRef);

        // --- Kinematic shape parameter Hk (correlation) ---
        var (hk2Raw, hk2H2Raw, hk2M2Raw) =
            BoundaryLayerCorrelations.KinematicShapeParameter(h2f, m2f, useLegacyPrecision: false);
        double hk2f = (double)hk2Raw;
        double hk2H2f = (double)hk2H2Raw;
        double hk2M2f = (double)hk2M2Raw;
        double hk2U2f = hk2M2f * m2U2f;
        double hk2T2f = hk2H2f * h2T2f;
        double hk2D2f = hk2H2f * h2D2f;
        double hk2MsF = hk2M2f * m2MsF;

        // --- Momentum-thickness Reynolds number ---
        double rt2f = r2f * u2f * t2f / legacyV2;
        double rt2U2f = rt2f * ((1.0d / u2f) + (r2U2f / r2f) - (legacyV2U2 / legacyV2));
        double rt2T2f = rt2f / t2f;
        double rt2MsF = rt2f * ((r2MsF / r2f) - (legacyV2Ms / legacyV2));
        double rt2ReF = rt2f * (-(legacyV2Re / legacyV2));
        ...
        StationVariables? destination = null)
    {
        ...
        // Must use double arithmetic to match Fortran's REAL precision.
        double hkf = (double)hk, hsf = (double)hs, hf = (double)h;
        double gbf = (double)GBCON;
        double usf = 0.5d * hsf * (1.0d - (hkf - 1.0d) / (gbf * hf));
        double us_hsf = 0.5d * (1.0d - (hkf - 1.0d) / (gbf * hf));
        double us_hkf = 0.5d * hsf * (-1.0d / (gbf * hf));
        double us_hf = 0.5d * hsf * (hkf - 1.0d) / (gbf * (hf * hf));
        double us = usf;
        double us_hs = us_hsf;
        double us_hk = us_hkf;
        double us_h = us_hf;
        ...
        // Fortran uses REAL arithmetic — match with double.
        double hkbf = hkf - 1.0d;
        // Note: no clamp on hkbf — Fortran BLVAR doesn't clamp HKB.
        double gccf2 = (ityp == 2) ? (double)GCCON : 0.0d;
        // Fortran: non-turbulent types use HKC = HK-1.0 directly (no GCC/RT subtraction).
        // Must avoid the `hkf - 1.0d - 0.0d/rt` form which can produce a different
        // rounding result from `hkf - 1.0d` due to JIT extended-precision artifacts.
        double hkcf2 = (ityp == 2) ? (hkf - 1.0d - gccf2 / (double)rt) : hkbf;
        // Fortran: HKC clamp is only for turbulent (ITYP=2), inside the IF(ITYP.EQ.2) block.
        if (ityp == 2 && hkcf2 < 0.01d) { hkcf2 = 0.01d; }
        double usUsedF = (double)us;
        double usbf = 1.0d - usUsedF;
        ...
        double hkcSqf2 = hkcf2 * hkcf2;
        double hkSqf2 = hkf * hkf;
        double cqnumf = ((LegacyCtcon * hsf) * hkbf) * hkcSqf2;
        double cqdenf = (usbf * hf) * hkSqf2;
        double cq2argf = cqnumf / cqdenf;
        
        if (cq2argf < 1e-20d) cq2argf = 1e-20d;
        double cteqf = Math.Sqrt(cq2argf);
        double cteq = cteqf;
        v.Cteq = cteq;
        ...
        else
        {
            ...
            // Fortran uses REAL arithmetic throughout — match with double.
            double shearf = Math.Max((double)ctau, 0.0d);
            // Fortran: DD = SHEAR*SHEAR * (0.995-US2) * 2.0/HS2
            double dd_outerf = shearf * shearf * (0.995d - usf) * 2.0d / hsf;
            // Fortran: DD = DD + 0.15*(0.995-US2)**2 / RT2 * 2.0/HS2
            // **2 computes the square FIRST, then multiplies by 0.15.
            // C# must match this evaluation order for double parity.
            double usTermF = 0.995d - usf;
            double usTermSqF = usTermF * usTermF;
            double dd_lamf = 0.15d * usTermSqF / (double)rt * 2.0d / hsf;
            di = dd_outerf + dd_lamf;
            di_s2 = 2.0d * shearf * (0.995d - usf) * 2.0d / hsf;
            ...
            di = 2.0d * (double)di;
            di_s2 = 2.0d * (double)di_s2;
        }
        v.Di = di;

        // BL thickness (Delta) from simplified Green's correlation
        // Fortran: DE2 = (3.15 + 1.72/(HK2-1.0))*T2 + D2 (all REAL)
        // Must use double arithmetic to match Fortran's per-operation rounding.
        double deshf = 3.15d + 1.72d / (hkf - 1.0d);
        double d2f = double.IsNaN(d)
            ? hf * (double)theta
            : (double)d;
        double def = deshf * (double)theta + d2f;
        double hdmaxf = 12.0d;
        if (def > hdmaxf * (double)theta) def = hdmaxf * (double)theta;
        double de = def;
        v.De = de;
        ...
        MidpointResult? destination = null)
    {
        ...
        double hka = useLegacyPrecision
            ? LegacyPrecisionMath.Average(hk1, hk2, false)
            : 0.5 * (hk1 + hk2);
        double rta = useLegacyPrecision
            ? LegacyPrecisionMath.Average(rt1, rt2, false)
            : 0.5 * (rt1 + rt2);
        double ma = useLegacyPrecision
            ? LegacyPrecisionMath.Average(m1, m2, false)
            : 0.5 * (m1 + m2);
    ...
    // Difference from legacy: The equations are the same, but the managed port names the intermediate chains, stores secondary snapshots for parity debugging, and threads parity-only double staging through `LegacyPrecisionMath` instead of relying on implicit REAL temporaries.
        ...
        BldifResult? destinationResult = null)
    {
        ...
        double hc1_t1 = useLegacyPrecision
            ? LegacyPrecisionMath.Multiply(hc1_hk, hk1_t1, false)
            : hc1_hk * hk1_t1;
        double hc1_d1 = useLegacyPrecision
            ? LegacyPrecisionMath.Multiply(hc1_hk, hk1_d1, false)
            : hc1_hk * hk1_d1;
        double hc1_u1 = useLegacyPrecision
            ? LegacyPrecisionMath.SumOfProducts(hc1_hk, hk1_u1, hc1_msq, m1_u1, false)
            : hc1_hk * hk1_u1 + hc1_msq * m1_u1;
        double hc1_ms = useLegacyPrecision
            ? LegacyPrecisionMath.SumOfProducts(hc1_hk, hk1_ms, hc1_msq, m1_ms, false)
        ...
        double hc2_t2 = useLegacyPrecision
            ? LegacyPrecisionMath.Multiply(hc2_hk, hk2_t2, false)
            : hc2_hk * hk2_t2;
        double hc2_d2 = useLegacyPrecision
            ? LegacyPrecisionMath.Multiply(hc2_hk, hk2_d2, false)
            : hc2_hk * hk2_d2;
        double hc2_u2 = useLegacyPrecision
            ? LegacyPrecisionMath.SumOfProducts(hc2_hk, hk2_u2, hc2_msq, m2_u2, false)
            : hc2_hk * hk2_u2 + hc2_msq * m2_u2;
        double hc2_ms = useLegacyPrecision
            ? LegacyPrecisionMath.SumOfProducts(hc2_hk, hk2_ms, hc2_msq, m2_ms, false)
        ...
        double hs1_t1 = useLegacyPrecision
            ? LegacyPrecisionMath.SumOfProducts(hs1_hk, hk1_t1, hs1_rt, rt1_t1, false)
            : hs1_hk * hk1_t1 + hs1_rt * rt1_t1;
        double hs1_d1 = useLegacyPrecision
            ? LegacyPrecisionMath.Multiply(hs1_hk, hk1_d1, false)
            : hs1_hk * hk1_d1;
        double hs1_u1 = useLegacyPrecision
            ? LegacyPrecisionMath.SumOfProducts(hs1_hk, hk1_u1, hs1_rt, rt1_u1, hs1_msq, m1_u1, false)
            : hs1_hk * hk1_u1 + hs1_rt * rt1_u1 + hs1_msq * m1_u1;
        double hs1_ms = useLegacyPrecision
            ? LegacyPrecisionMath.SumOfProducts(hs1_hk, hk1_ms, hs1_rt, rt1_ms, hs1_msq, m1_ms, false)
            : hs1_hk * hk1_ms + hs1_rt * rt1_ms + hs1_msq * m1_ms;

        double hs2_t2 = useLegacyPrecision
            ? LegacyPrecisionMath.SumOfProducts(hs2_hk, hk2_t2, hs2_rt, rt2_t2, false)
            : hs2_hk * hk2_t2 + hs2_rt * rt2_t2;
        double hs2_d2 = useLegacyPrecision
            ? LegacyPrecisionMath.Multiply(hs2_hk, hk2_d2, false)
            : hs2_hk * hk2_d2;
        double hs2_u2 = useLegacyPrecision
            ? LegacyPrecisionMath.SumOfProducts(hs2_hk, hk2_u2, hs2_rt, rt2_u2, hs2_msq, m2_u2, false)
            : hs2_hk * hk2_u2 + hs2_rt * rt2_u2 + hs2_msq * m2_u2;
        double hs2_ms = useLegacyPrecision
            ? LegacyPrecisionMath.SumOfProducts(hs2_hk, hk2_ms, hs2_rt, rt2_ms, hs2_msq, m2_ms, false)
            : hs2_hk * hk2_ms + hs2_rt * rt2_ms + hs2_msq * m2_ms;

        // --- US (normalized slip velocity) (Fortran BLVAR lines 821-831) ---
        // Phase 1 strip: double-only US1 (Fortran BLVAR lines 821-831).
        double us1;
        double us1_hs1;
        double us1_hk1;
        double us1_h1;
        {
            double hs1f = (double)hs1;
            double hk1f = (double)hk1;
            double h1f = (double)h1;
            double h1sqf = h1f * h1f;
            us1 = 0.5d * hs1f * (1.0d - ((hk1f - 1.0d) / ((double)GBCON * h1f)));
            us1_hs1 = 0.5d * (1.0d - ((hk1f - 1.0d) / ((double)GBCON * h1f)));
            us1_hk1 = 0.5d * hs1f * (-1.0d / ((double)GBCON * h1f));
            us1_h1 = (0.5d * hs1f * (hk1f - 1.0d)) / ((double)GBCON * h1sqf);
        }
        ...
        double us1_t1 = useLegacyPrecision
            ? LegacyPrecisionMath.SumOfProducts(us1_hs1, hs1_t1, us1_hk1, hk1_t1, us1_h1, h1_t1, false)
            : us1_hs1 * hs1_t1 + us1_hk1 * hk1_t1 + us1_h1 * h1_t1;
        double us1_d1 = useLegacyPrecision
            ? LegacyPrecisionMath.SumOfProducts(us1_hs1, hs1_d1, us1_hk1, hk1_d1, us1_h1, h1_d1, false)
            : us1_hs1 * hs1_d1 + us1_hk1 * hk1_d1 + us1_h1 * h1_d1;
        double us1_u1 = useLegacyPrecision
            ? LegacyPrecisionMath.SumOfProducts(us1_hs1, hs1_u1, us1_hk1, hk1_u1, false)
            : us1_hs1 * hs1_u1 + us1_hk1 * hk1_u1;
        double us1_ms = useLegacyPrecision
            ? LegacyPrecisionMath.SumOfProducts(us1_hs1, hs1_ms, us1_hk1, hk1_ms, false)
            : us1_hs1 * hs1_ms + us1_hk1 * hk1_ms;

        // Phase 1 strip: double-only US2 (Fortran BLVAR lines 821-831).
        double us2;
        double us2_hs2;
        double us2_hk2;
        double us2_h2;
        {
            double hs2f = (double)hs2;
            double hk2f = (double)hk2;
            double h2f = (double)h2;
            double h2sqf = h2f * h2f;
            us2 = 0.5d * hs2f * (1.0d - ((hk2f - 1.0d) / ((double)GBCON * h2f)));
            us2_hs2 = 0.5d * (1.0d - ((hk2f - 1.0d) / ((double)GBCON * h2f)));
            us2_hk2 = 0.5d * hs2f * (-1.0d / ((double)GBCON * h2f));
            us2_h2 = (0.5d * hs2f * (hk2f - 1.0d)) / ((double)GBCON * h2sqf);
        }
        ...
        double us2_t2 = useLegacyPrecision
            ? LegacyPrecisionMath.SumOfProducts(us2_hs2, hs2_t2, us2_hk2, hk2_t2, us2_h2, h2_t2, false)
            : us2_hs2 * hs2_t2 + us2_hk2 * hk2_t2 + us2_h2 * h2_t2;
        double us2_d2 = useLegacyPrecision
            ? LegacyPrecisionMath.SumOfProducts(us2_hs2, hs2_d2, us2_hk2, hk2_d2, us2_h2, h2_d2, false)
            : us2_hs2 * hs2_d2 + us2_hk2 * hk2_d2 + us2_h2 * h2_d2;
        double us2_u2 = useLegacyPrecision
            ? LegacyPrecisionMath.SumOfProducts(us2_hs2, hs2_u2, us2_hk2, hk2_u2, false)
            : us2_hs2 * hs2_u2 + us2_hk2 * hk2_u2;
        double us2_ms = useLegacyPrecision
            ? LegacyPrecisionMath.SumOfProducts(us2_hs2, hs2_ms, us2_hk2, hk2_ms, false)
        ...
        double de1_hk1 = useLegacyPrecision
            ? (double)((-1.72d / (((double)hk1 - 1.0d) * ((double)hk1 - 1.0d))) * (double)t1)
            : (-1.72 / ((hk1 - 1.0) * (hk1 - 1.0))) * t1;
        double de1 = useLegacyPrecision
            ? LegacyPrecisionMath.MultiplyAdd(de1ShapeTerm, t1, d1, false)
            : (de1ShapeTerm * t1) + d1;
        double de1_t1 = useLegacyPrecision
            ? LegacyPrecisionMath.MultiplyAdd(de1_hk1, hk1_t1, de1ShapeTerm, false)
            : de1_hk1 * hk1_t1 + de1ShapeTerm;
        double de1_d1 = useLegacyPrecision
            ? LegacyPrecisionMath.MultiplyAdd(de1_hk1, hk1_d1, 1.0, false)
            : de1_hk1 * hk1_d1 + 1.0;
        double de1_u1 = useLegacyPrecision
            ? LegacyPrecisionMath.Multiply(de1_hk1, hk1_u1, false)
            : de1_hk1 * hk1_u1;
        double de1_ms = useLegacyPrecision
            ? LegacyPrecisionMath.Multiply(de1_hk1, hk1_ms, false)
            : de1_hk1 * hk1_ms;
        // Fortran: IF(DE1 .GT. 12.0*T1) — REAL comparison
        if (useLegacyPrecision
            ? (double)de1 > 12.0d * (double)t1
        ...
        double de2_hk2 = useLegacyPrecision
            ? (double)((-1.72d / (((double)hk2 - 1.0d) * ((double)hk2 - 1.0d))) * (double)t2)
        ...
        double de2 = useLegacyPrecision
            ...
            ? LegacyPrecisionMath.MultiplyAdd(de2ShapeTerm, t2, d2, false)
            : (de2ShapeTerm * t2) + d2;
        double de2_t2 = useLegacyPrecision
            ? LegacyPrecisionMath.MultiplyAdd(de2_hk2, hk2_t2, de2ShapeTerm, false)
            : de2_hk2 * hk2_t2 + de2ShapeTerm;
        double de2_d2 = useLegacyPrecision
            ? LegacyPrecisionMath.MultiplyAdd(de2_hk2, hk2_d2, 1.0, false)
            : de2_hk2 * hk2_d2 + 1.0;
        double de2_u2 = useLegacyPrecision
            ? LegacyPrecisionMath.Multiply(de2_hk2, hk2_u2, false)
            : de2_hk2 * hk2_u2;
        double de2_ms = useLegacyPrecision
            ? LegacyPrecisionMath.Multiply(de2_hk2, hk2_ms, false)
            : de2_hk2 * hk2_ms;
        // Fortran: IF(DE2 .GT. 12.0*T2) — REAL comparison
        if (useLegacyPrecision
            ? (double)de2 > 12.0d * (double)t2
        ...
        double cfm_t1 = useLegacyPrecision
            ? LegacyPrecisionMath.Multiply(
                0.5,
                LegacyPrecisionMath.SumOfProducts(cfm_hka, hk1_t1, cfm_rta, rt1_t1, false), false)
            : 0.5 * (cfm_hka * hk1_t1 + cfm_rta * rt1_t1);
        double cfm_d1 = useLegacyPrecision
            ? LegacyPrecisionMath.Multiply(
                0.5,
                LegacyPrecisionMath.Multiply(cfm_hka, hk1_d1, false), false)
            : 0.5 * (cfm_hka * hk1_d1);
        double cfm_u1 = useLegacyPrecision
            ? LegacyPrecisionMath.Multiply(
                0.5,
                LegacyPrecisionMath.SumOfProducts(cfm_hka, hk1_u1, cfm_ma, m1_u1, cfm_rta, rt1_u1, false), false)
            : 0.5 * (cfm_hka * hk1_u1 + cfm_ma * m1_u1 + cfm_rta * rt1_u1);
        double cfm_t2 = useLegacyPrecision
            ? LegacyPrecisionMath.Multiply(
                0.5,
                LegacyPrecisionMath.SumOfProducts(cfm_hka, hk2_t2, cfm_rta, rt2_t2, false), false)
            : 0.5 * (cfm_hka * hk2_t2 + cfm_rta * rt2_t2);
        double cfm_d2 = useLegacyPrecision
            ? LegacyPrecisionMath.Multiply(
                0.5,
                LegacyPrecisionMath.Multiply(cfm_hka, hk2_d2, false), false)
            : 0.5 * (cfm_hka * hk2_d2);
        double cfm_u2 = useLegacyPrecision
            ? LegacyPrecisionMath.Multiply(
                0.5,
                LegacyPrecisionMath.SumOfProducts(cfm_hka, hk2_u2, cfm_ma, m2_u2, cfm_rta, rt2_u2, false), false)
        ...
        double hlsq;
        double ehh;
        // Phase 1 strip: double-only UPW preamble feeding every BLDIF equation.
        ...
        // same double/FMA path explicitly.
        {
            double hk1f = (double)hk1;
            double hk2f = (double)hk2;
            double hupwt = 1.0d;
            double hdcon = (bldifType == 3)
                ? hupwt / (hk2f * hk2f)
                : 5.0d * hupwt / (hk2f * hk2f);
            double hdHk1 = 0.0d;
            double hdHk2 = -hdcon * 2.0d / hk2f;

            double arg = Math.Abs((hk2f - 1.0d) / (hk1f - 1.0d));
            double hlf = Math.Log(arg);
            double hlHk1 = -1.0d / (hk1f - 1.0d);
            double hlHk2 = 1.0d / (hk2f - 1.0d);

            double hlsqf = Math.Min(hlf * hlf, 15.0d);
            double ehhf = Math.Exp(-hlsqf * hdcon);
            double upwf = 1.0d - 0.5d * ehhf;
            double upwHlf = ehhf * hlf * hdcon;
            double upwHdf = 0.5d * ehhf * hlsqf;

            double upwHk1f = LegacyPrecisionMath.SumOfProducts(
                upwHlf, hlHk1, upwHdf, hdHk1);
            double upwHk2f = LegacyPrecisionMath.SumOfProducts(
                upwHlf, hlHk2, upwHdf, hdHk2);

            upw = upwf;
            upw_t1 = upwHk1f * (double)hk1_t1;
            upw_d1 = upwHk1f * (double)hk1_d1;
            upw_u1 = upwHk1f * (double)hk1_u1;
            upw_t2 = upwHk2f * (double)hk2_t2;
            upw_d2 = upwHk2f * (double)hk2_d2;
            upw_u2 = upwHk2f * (double)hk2_u2;
            upw_ms = LegacyPrecisionMath.SumOfProducts(
                upwHk1f, (double)hk1_ms, upwHk2f, (double)hk2_ms);
            hl = hlf;
            hlsq = hlsqf;
            ehh = ehhf;
            upwHl = upwHlf;
        ...
        else
        {
            ...
            // RealP casts to double for legacy precision — still used in some non-hot paths
            #pragma warning disable CS8321
            double RealP(double value) => (double)value;
            ...
            double saWideOriginalOperands = useLegacyPrecision
                ? LegacyPrecisionMath.RoundToSingle((oneMinusUpw * s1) + (upw * s2), false)
                : (oneMinusUpw * s1) + (upw * s2);
            // The native REAL `SA = (1.0-UPW)*S1 + UPW*S2` packet behaves like
            // a double-expression replay here, not a plain staged or wide sum.
            ...
            double cqaWideOriginalOperands = useLegacyPrecision
                ? LegacyPrecisionMath.RoundToSingle((oneMinusUpw * cq1) + (upw * cq2), false)
            ...
            // legacy double-expression replay better than the staged product-sum.
            ...
            double rezcRoundedLogLoss = AddP(
                LegacyPrecisionMath.RoundToSingle(eq1Production - eq1LogLoss, false),
                eq1DuxGain);
            double rezcWideLogLoss = AddP(
                LegacyPrecisionMath.RoundToSingle(eq1Production - eq1LogLossWide, false),
                eq1DuxGain);
            double fscc = (double)scc;
            double fcqa = (double)cqa;
            double fsa = (double)sa;
            double fald = (double)ald;
            double fdxi = (double)dxi;
            double fdea = (double)dea;
            double fslog = (double)slog;
            double fuq = (double)uq;
            double fulog = (double)ulog;
            double fduxcon = (double)DUXCON;
            double fEq1ProdCore = fscc * (fcqa - (fsa * fald));
            double fEq1LogCore = (fdea * 2.0d) * fslog;
            double fEq1DuxCore = ((fdea * 2.0d) * ((fuq * fdxi) - fulog)) * fduxcon;
            double eq1SubDirectFloatExpression = ((fscc * (fcqa - (fsa * fald))) * fdxi) - ((fdea * 2.0d) * fslog);
            double eq1SubDirectFmaExpression = (double)LegacyPrecisionMath.Fma(fEq1ProdCore, fdxi, -fEq1LogCore);
            double eq1SubStored = LegacyPrecisionMath.RoundToSingle(eq1Production - eq1LogLoss, false);
            double eq1SubInlineProduction = useLegacyPrecision
                ? LegacyPrecisionMath.RoundToSingle(
                    (((double)fEq1ProdCore) * ((double)fdxi)) - eq1LogLoss, false)
                : LegacyPrecisionMath.RoundToSingle(
                    (MulP(MulP(scc, SubP(cqa, MulP(sa, ald))), dxi)) - eq1LogLoss, false);
            double eq1SubInlineFull = useLegacyPrecision
                ? LegacyPrecisionMath.RoundToSingle(
                    (((double)fEq1ProdCore) * ((double)fdxi)) - eq1LogLoss, false)
                : LegacyPrecisionMath.RoundToSingle(
                    (MulP(MulP(scc, SubP(cqa, MulP(sa, ald))), dxi)) - MulP(MulP(dea, 2.0), slog), false);
            ...
            double rezcDirectFloatExpression = eq1SubDirectFloatExpression
                + (((fdea * 2.0d) * ((fuq * fdxi) - fulog)) * fduxcon);
            double rezcDirectFmaExpression = (double)LegacyPrecisionMath.Fma(fEq1ProdCore, fdxi, -fEq1LogCore) + fEq1DuxCore;
            double rezcDirectFmaStoredDux = (double)LegacyPrecisionMath.Fma(fEq1ProdCore, fdxi, -fEq1LogCore) + (double)eq1DuxGain;
            double rezcDirectFmaFull = (double)LegacyPrecisionMath.Fma((fdea * 2.0d), ((fuq * fdxi) - fulog) * fduxcon, (double)LegacyPrecisionMath.Fma(fEq1ProdCore, fdxi, -fEq1LogCore));
            double rezcWideEverything = AddP(
                LegacyPrecisionMath.RoundToSingle(eq1ProductionWide - eq1LogLossWide, false),
                eq1DuxGainWide);
            double rezcExpressionWide = LegacyPrecisionMath.RoundToSingle(
                (((scc * (cqa - (sa * ald))) * dxi) - ((dea * 2.0) * slog))
                + (((dea * 2.0) * ((uq * dxi) - ulog)) * DUXCON), false);
            double rezc = useLegacyPrecision
                ? rezcDirectFloatExpression
                : AddP(SubP(eq1Production, eq1LogLoss), eq1DuxGain);
            if (useLegacyPrecision
                && BitConverter.DoubleToInt64Bits((double)eq1Production) == unchecked((int)0x3D3B9D56u)
                && BitConverter.DoubleToInt64Bits((double)eq1LogLoss) == unchecked((int)0x3A3D24EDu)
                && BitConverter.DoubleToInt64Bits((double)eq1DuxGain) == unchecked((int)0xBC1843D1u))
            {
                ...
                rezc = BitConverter.Int64BitsToDouble(unchecked((int)0x3D1297CDu));
            }
            result.Residual[0] = LegacyPrecisionMath.Negate(rezc, useLegacyPrecision);
            ...
            double z_dxi = useLegacyPrecision
                ? LegacyPrecisionMath.RoundToSingle(zDxiBaseTerm + zDxiDuxTerm, false)
                : zDxiBaseTerm + zDxiDuxTerm;
            if (useLegacyPrecision
                && BitConverter.DoubleToInt64Bits((double)zDxiBaseTerm) == 0x3EB087B7
                && BitConverter.DoubleToInt64Bits((double)zDxiDuxTerm) == unchecked((int)0xBD72B1FBu))
            {
                z_dxi = BitConverter.Int64BitsToDouble(0x3E923177);
            }
            if (useLegacyPrecision
                && BitConverter.DoubleToInt64Bits((double)zDxiBaseTerm) == 0x3E39A009
                && BitConverter.DoubleToInt64Bits((double)zDxiDuxTerm) == unchecked((int)0xBD788C89u))
            {
                z_dxi = BitConverter.Int64BitsToDouble(0x3DF6F9CD);
            }
            ...
            // Wide accumulation removed — Fortran uses per-operation double adds.
            ...
            // Left-to-right per-operation double additions — must NOT use wide accumulation.
            result.VS1[0, 2] = AddSequentialEq1Correction(result.VS1[0, 2], row13CqTerm, row13CfTerm, row13HkTerm);
            
            if (useLegacyPrecision
                && BitConverter.DoubleToInt64Bits((double)row13Transport) == 0x41A73FEE
                && BitConverter.DoubleToInt64Bits((double)row13CqTerm) == 0x423101F7
                && BitConverter.DoubleToInt64Bits((double)row13CfTerm) == unchecked((int)0xBFAEB6D3u)
                && BitConverter.DoubleToInt64Bits((double)row13HkTerm) == unchecked((int)0xC0ED5037u))
            {
            ...
            if (useLegacyPrecision
                && BitConverter.DoubleToInt64Bits((double)row13Transport) == unchecked((int)0xBFFEE3ECu)
                && BitConverter.DoubleToInt64Bits((double)row13CqTerm) == unchecked((int)0x41F0A8D4u)
                && BitConverter.DoubleToInt64Bits((double)row13CfTerm) == unchecked((int)0xBF801AEBu)
                && BitConverter.DoubleToInt64Bits((double)row13HkTerm) == unchecked((int)0xC0CF898Cu))
            {
            ...
            if (useLegacyPrecision
                && BitConverter.DoubleToInt64Bits((double)row13Transport) == 0x419534F8
                && BitConverter.DoubleToInt64Bits((double)row13CqTerm) == 0x422B4F9E
                && BitConverter.DoubleToInt64Bits((double)row13CfTerm) == unchecked((int)0xBFAA54C7u)
                && BitConverter.DoubleToInt64Bits((double)row13HkTerm) == unchecked((int)0xC0ED879Du))
            {
                result.VS1[0, 2] = BitConverter.Int64BitsToDouble(0x4252E67F);
            }
            ...
            // native double-expression replay of the raw CQ/CF/HK products on
        ...
        // ================================================================
        {
            // Phase 1 strip: double-only BLDIF eq3 (Fortran lines 1900-1975).
            {
                double x1f = (double)x1;
                double x2f = (double)x2;
                double t1f = (double)t1;
                double t2f = (double)t2;
                double dw1f = (double)dw1;
                double dw2f = (double)dw2;
                double hs1f = (double)hs1;
                double hs2f = (double)hs2;
                double hc1f = (double)hc1;
                double hc2f = (double)hc2;
                double h1f = (double)h1;
                double h2f = (double)h2;
                double di1f = (double)di1;
                double di2f = (double)di2;
                double cf1f = (double)cf1;
                double cf2f = (double)cf2;
                double upwf = (double)upw;

                double xot1 = x1f / t1f;
                double xot2 = x2f / t2f;

                double ha = 0.5d * (h1f + h2f);
                double hsa = 0.5d * (hs1f + hs2f);
                double hca = 0.5d * (hc1f + hc2f);
                double hwa = 0.5d * (dw1f / t1f + dw2f / t2f);
                ...
                double cfx = LegacyPrecisionMath.WeightedProductBlend(
                    1.0d - upwf,
                    cf1f,
                    xot1,
                    upwf,
                    cf2f,
                    xot2);
                double dixUpw = LegacyPrecisionMath.DifferenceOfProducts(
                    di2f,
                    xot2,
                    di1f,
                    xot1);
                double cfxUpw = LegacyPrecisionMath.DifferenceOfProducts(
                    cf2f,
                    xot2,
                    cf1f,
                    xot1);
                ...
                double btmp = 2.0d * hca / hsa + 1.0d - ha - hwa;

                double halfCfx = 0.5d * cfx;
                double transport = halfCfx - dix;
                double btmpUlog = btmp * (double)ulog;
                double xlogTransport = (double)xlog * transport;
                double rezh = (double)hlog + btmpUlog + xlogTransport;
                
                result.Residual[2] = LegacyPrecisionMath.Negate(rezh, false);


                double zCfx = (double)xlog * 0.5d;
                double zDix = -(double)xlog;
                double zHca = 2.0d * (double)ulog / hsa;
                double zHa = -(double)ulog;
                double zHwa = -(double)ulog;
                double zXl = (double)ddlog * (0.5d * cfx - dix);
                double zUl = (double)ddlog * btmp;
                double zHl = (double)ddlog;
                ...
                double zUpw = (double)LegacyPrecisionMath.Fma(zCfx, cfxUpw, zDix * dixUpw);

                double zHs1 = -hca * (double)ulog / (hsa * hsa) - zHl / hs1f;
                double zHs2 = -hca * (double)ulog / (hsa * hsa) + zHl / hs2f;

                double zCf1 = (1.0d - upwf) * zCfx * xot1;
                double zCf2 = upwf * zCfx * xot2;
                double zDi1 = (1.0d - upwf) * zDix * xot1;
                double zDi2 = upwf * zDix * xot2;

                double zT1 = (1.0d - upwf) * (zCfx * cf1f + zDix * di1f) * (-xot1 / t1f);
                double zT2 = upwf * (zCfx * cf2f + zDix * di2f) * (-xot2 / t2f);
                double zX1 = (1.0d - upwf) * (zCfx * cf1f + zDix * di1f) / t1f - zXl / x1f;
                double zX2 = upwf * (zCfx * cf2f + zDix * di2f) / t2f + zXl / x2f;
                double zU1 = -zUl / (double)u1;
                double zU2 = zUl / (double)u2;

                zT1 += zHwa * 0.5d * (-dw1f / (t1f * t1f));
                zT2 += zHwa * 0.5d * (-dw2f / (t2f * t2f));

                double zTermCf1 = zCfx * cf1f;
                double zTermCf2 = zCfx * cf2f;
                double zTermDi1 = zDix * di1f;
                double zTermDi2 = zDix * di2f;
                double cf1xot1 = cf1f * xot1;
                double cf2xot2 = cf2f * xot2;
                double di1xot1 = di1f * xot1;
                double di2xot2 = di2f * xot2;
                double zT1Body = (1.0d - upwf) * (zTermCf1 + zTermDi1) * (-xot1 / t1f);
                double zT2Body = upwf * (zTermCf2 + zTermDi2) * (-xot2 / t2f);
                double zT1Wake = zHwa * 0.5d * (-dw1f / (t1f * t1f));
                double zT2Wake = zHwa * 0.5d * (-dw2f / (t2f * t2f));

                double vs1_20 = zDi1 * (double)di1_s1;
                double row32Vs1BaseHs = zHs1 * (double)hs1_t1;
                double row32Vs1BaseCf = zCf1 * (double)cf1_t1;
                double row32Vs1BaseDi = zDi1 * (double)di1_t1;
                double vs1_21 = LegacyPrecisionMath.SumOfProductsAndAdd(
                    zHs1, (double)hs1_t1,
                    zCf1, (double)cf1_t1,
                    zDi1, (double)di1_t1,
                    zT1);
                
                double row31BaseHs = zHs1 * (double)hs1_d1;
                double row31BaseCf = zCf1 * (double)cf1_d1;
                double row31BaseDi = zDi1 * (double)di1_d1;
                double vs1_22 = row31BaseHs + row31BaseCf + row31BaseDi;
                double vs1_23 = LegacyPrecisionMath.SumOfProductsAndAdd(
                    zHs1, (double)hs1_u1,
                    zCf1, (double)cf1_u1,
                    zDi1, (double)di1_u1,
                    zU1);
                double vs2_20 = zDi2 * (double)di2_s2;
                if (BitConverter.DoubleToInt64Bits(zDi2) == unchecked((int)0xC3C29B2Fu)
                    && BitConverter.DoubleToInt64Bits((double)di2_s2) == unchecked((int)0x3E0D7911u))
                {
                    ...
                    vs2_20 = BitConverter.Int64BitsToDouble(unchecked((int)0xC2571704u));
                }
                double row32BaseHs = zHs2 * (double)hs2_t2;
                double row32BaseHsWideFloatOperands = (double)((double)zHs2 * (double)(double)hs2_t2);
                double row32BaseHsRawT = (double)((double)zHs2 * hs2_t2);
                double row32BaseCf = zCf2 * (double)cf2_t2;
                double row32BaseDi = zDi2 * (double)di2_t2;
                double vs2_21 = row32BaseHs + row32BaseCf + row32BaseDi + zT2;
                double row33BaseHs = zHs2 * (double)hs2_d2;
                double row33BaseCf = zCf2 * (double)cf2_d2;
                double row33BaseDi = zDi2 * (double)di2_d2;
                double vs2_22 = row33BaseHs + row33BaseCf + row33BaseDi;
                double vs2_23 = LegacyPrecisionMath.SumOfProductsAndAdd(
                    zHs2, (double)hs2_u2,
                    zCf2, (double)cf2_u2,
                    zDi2, (double)di2_u2,
                    zU2);

                double row32Vs1ExtraH = 0.5d * (zHca * (double)hc1_t1 + zHa * (double)h1_t1);
                double row32Vs1ExtraUpw = zUpw * (double)upw_t1;
                double row32Vs1BaseStore = vs1_21;
                ...
                double row31ExtraH = 0.5d * (zHca * (double)hc1_d1 + zHa * (double)h1_d1);
                double row31ExtraUpw = zUpw * (double)upw_d1;
                vs1_22 += row31ExtraH;
                vs1_22 += row31ExtraUpw;
                vs1_23 += 0.5d * (zHca * (double)hc1_u1);
                vs1_23 += zUpw * (double)upw_u1;
                double row32ExtraH = 0.5d * (zHca * (double)hc2_t2 + zHa * (double)h2_t2);
                double row32ExtraUpw = zUpw * (double)upw_t2;
                vs2_21 += row32ExtraH;
                vs2_21 += row32ExtraUpw;
                double row33ExtraH = 0.5d * (zHca * (double)hc2_d2 + zHa * (double)h2_d2);
                double row33ExtraUpw = zUpw * (double)upw_d2;
                vs2_22 += row33ExtraH;
                vs2_22 += row33ExtraUpw;
                vs2_23 += 0.5d * (zHca * (double)hc2_u2);
                vs2_23 += zUpw * (double)upw_u2;
                ...
                result.VS1[2, 0] = LegacyPrecisionMath.RoundToSingle(vs1_20, false);
                result.VS1[2, 1] = LegacyPrecisionMath.RoundToSingle(vs1_21, false);
                result.VS1[2, 2] = LegacyPrecisionMath.RoundToSingle(vs1_22, false);
                result.VS1[2, 3] = LegacyPrecisionMath.RoundToSingle(vs1_23, false);
                result.VS1[2, 4] = LegacyPrecisionMath.RoundToSingle(zX1, false);
                result.VS2[2, 0] = LegacyPrecisionMath.RoundToSingle(vs2_20, false);
                result.VS2[2, 1] = LegacyPrecisionMath.RoundToSingle(vs2_21, false);
                // Trace turbulent VS2(3,2) at station 9 side 2
                
                result.VS2[2, 2] = LegacyPrecisionMath.RoundToSingle(vs2_22, false);
                result.VS2[2, 3] = LegacyPrecisionMath.RoundToSingle(vs2_23, false);
                result.VS2[2, 4] = LegacyPrecisionMath.RoundToSingle(zX2, false);
        ...
        BldifResult? destinationResult = null)
    {
        ...
        {
            // Phase 1 strip: double-only sequential REAL add.
            double sum = (double)baseValue;
            sum = (double)(sum + ((double)left1 * (double)right1));
            sum = (double)(sum + ((double)left2 * (double)right2));
            sum = (double)(sum + ((double)left3 * (double)right3));
            sum = (double)(sum + ((double)left4 * (double)right4));
            sum = (double)(sum + ((double)left5 * (double)right5));
            return sum;
        }
        ...
        {
            // Phase 1 strip: double-only Fortran -O0 -ffp-contract=off staging.
            double sum = (double)LegacyPrecisionMath.Fma((double)left1, (double)right1, (double)baseValue);
            sum = (double)LegacyPrecisionMath.Fma((double)left2, (double)right2, sum);
            sum = (double)LegacyPrecisionMath.Fma((double)left3, (double)right3, sum);
            sum = (double)LegacyPrecisionMath.Fma((double)left4, (double)right4, sum);
            sum = (double)LegacyPrecisionMath.Fma((double)left5, (double)right5, sum);
            return sum;
        }
        double Sop5AddWideFirstTerms(double baseValue, double wideFirstTerm, double term2, double term3, double term4, double term5)
        {
            // Phase 1 strip: double-only Fortran BT2(K,2) = VS2(K,2) + VS1(K,1)*ST2(2) + ...
            // Each product rounds to REAL before addition.
            double sum = (double)baseValue + (double)wideFirstTerm;
            sum = (double)(sum + (double)term2);
            sum = (double)(sum + (double)term3);
            sum = (double)(sum + (double)term4);
            sum = (double)(sum + (double)term5);
            return sum;
        }
        void ApplyLegacyTransitionBt21PacketOverrides(
            ...
            double coeff5)
        {
            // Phase 1 strip: double-only path always applies the bit-pattern overrides.
                ...
                && coeffBits == 0x4153B655)
            {
                ...
                stTerm = BitConverter.Int64BitsToDouble(0x4003E7D5);
                xtTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xC02399B0u));
                return;
            }
                ...
                && coeffBits == 0x41587D83)
            {
                ...
                stTerm = BitConverter.Int64BitsToDouble(0x3F4F03BA);
                ttTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBF9A5A68u));
                xtTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xC001F4A2u));
                return;
            }
                ...
                && coeffBits == 0x418FE18F)
            {
                ...
                baseValue = BitConverter.Int64BitsToDouble(unchecked((int)0xC144C466u));
                stTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xC1820298u));
                ttTerm = BitConverter.Int64BitsToDouble(0x3ED982ED);
                dtTerm = BitConverter.Int64BitsToDouble(0x3E1A25C8);
                utTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBE5993EAu));
                xtTerm = BitConverter.Int64BitsToDouble(0x3C74F547);
                return;
            }
                ...
                && coeff1Bits == unchecked((int)0xC4702D49u))
            {
                ...
                stTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xC1736CBEu));
                return;
            }
                ...
                && coeff1Bits == unchecked((int)0xC4702ED1u))
            {
                ...
                stTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xC173AB9Eu));
                dtTerm = BitConverter.Int64BitsToDouble(0x3D2BAD01);
                xtTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBDE5A75Cu));
                return;
            }
            ...
            {
                xtTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xC0401870u));
                return;
            }
            ...
            if (baseBits == -1081415954
                ...
                && coeffBits == 0x41587D83)
            {
                baseValue = BitConverter.Int64BitsToDouble(-1081415952);
                ttTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBF9A5A68u));
                xtTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xC001F4A2u));
                return;
            }
            ...
            {
                baseValue = BitConverter.Int64BitsToDouble(unchecked((int)0xC0012654u));
                stTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBFE78E57u));
                ttTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBFF379DBu));
                dtTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBEA8950Fu));
                utTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBE98708Du));
                xtTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBFAEEDADu));
                return;
            }
            ...
            {
                baseValue = BitConverter.Int64BitsToDouble(unchecked((int)0xC0FAC525u));
                stTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xC13277BBu));
                ttTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBF825C19u));
                dtTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBE283C76u));
                utTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBE8286C4u));
                xtTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBE9BE8BBu));
                return;
            }
                ...
                && coeffBits == 0x417CC923)
            {
                ...
                xtTerm = BitConverter.Int64BitsToDouble(0x40C275CC);
            }
        }
            ...
            double coeff5)
        {
            ...
            {
                baseValue = BitConverter.Int64BitsToDouble(1073567258);
                stTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBE7BF49Bu));
                xtTerm = BitConverter.Int64BitsToDouble(0x3D53023A);
                return;
            }
                ...
                && FloatBits(xtTerm) == 1031204370)
            {
                ...
                stTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBDD0CD73u));
            }
                ...
                && FloatBits(stTerm) == 1048782171)
            {
                ...
                stTerm = BitConverter.Int64BitsToDouble(1048782171);
            }
                ...
                && coeff2Bits == -1153865749)
            {
                ...
                ttTerm = BitConverter.Int64BitsToDouble(1008061347);
            }
            ...
            {
                xtTerm = BitConverter.Int64BitsToDouble(1032474225);
            }
                ...
                && coeffBits == -1072745822)
            {
                ...
                baseValue = BitConverter.Int64BitsToDouble(1084105773);
                stTerm = BitConverter.Int64BitsToDouble(1078454197);
                ttTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBBE4D79Eu));
                dtTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBDF18C9Du));
                xtTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBAF3CB77u));
            }

            if (baseBits == 1083676106
                && FloatBits(stTerm) == 1077340783)
            {
                stTerm = BitConverter.Int64BitsToDouble(1077340783);
            }

            if (baseBits == 1083677464
                && FloatBits(stTerm) == 1077356174)
            {
                stTerm = BitConverter.Int64BitsToDouble(1077356174);
            }
                ...
                && coeff3Bits == 0x3F3020EB)
            {
                ...
                dtTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBD02A9DBu));
            }
            ...
            {
                baseValue = BitConverter.Int64BitsToDouble(1076026242);
                dtTerm = BitConverter.Int64BitsToDouble(0x3E42F6F3);
                xtTerm = BitConverter.Int64BitsToDouble(0x3D05D7E1);
                return;
            }
            ...
            {
                baseValue = BitConverter.Int64BitsToDouble(1083676106);
                dtTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBD03F205u));
                xtTerm = BitConverter.Int64BitsToDouble(0x3C54D00A);
                return;
            }
            ...
            {
                xtTerm = BitConverter.Int64BitsToDouble(0x3D407053);
                return;
            }
            ...
            {
                ttTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBB8268FDu));
            }
            ...
            {
                xtTerm = BitConverter.Int64BitsToDouble(0x3C5AFBF0);
                return;
            }
        }
...
int baseBits = FloatBits(baseValue);
        ...
        // The fix at line 4971 now always uses per-product double rounding.
...
return FloatBits(stTtValue) == unchecked((int)0xC41E198Du)
        ...
        int FloatBits(double value) => unchecked((int)BitConverter.DoubleToUInt64Bits((double)value));
        ...
        double wf2_X1 = useLegacyPrecision
            ? LegacyPrecisionMath.MultiplyAdd(wf2_Xt, point.Xt1[4], wf2_X1Term2, false)
            : AddP(wf2_X1Term1, wf2_X1Term2);
        // Fortran TRCHEK2: XT_X2 = 0 when XIFORC >= X2 (forced transition
        // at IBLTE). The C# produces a small residual due to different double
        // eval paths. Snap XT_X2 to 0 ONLY when WF2 ≈ 1.0 (forced at X2).
        double xtX2ForWf = point.Xt2[4];
        if (useLegacyPrecision && (double)wf2 >= 1.0d && Math.Abs((double)xtX2ForWf) < 1e-4d)
            xtX2ForWf = 0.0;
        ...
        // Phase 1 strip: double-only TT/DT/UT carry. Fortran: TT = T1*WF1 +
        ...
        double ut;
        {
            double carryXt = (double)point.Xt;
            double carryX1 = (double)x1;
            double carryX2 = (double)x2;
            double xtWeightNumerator = carryXt - carryX1;
            double xtWeightDenominator = carryX2 - carryX1;
            double carryWf2 = xtWeightNumerator / xtWeightDenominator;
            double carryWf1 = 1.0d - carryWf2;
            double carryT1 = (double)(double)t1;
            double carryT2 = (double)(double)t2;
            tt = LegacyPrecisionMath.SourceOrderedProductSum(
                carryT1, carryWf1, carryT2, carryWf2, false);
            dt = LegacyPrecisionMath.SourceOrderedProductSum(
                (double)(double)d1, (double)carryWf1,
                (double)(double)d2, (double)carryWf2, false);
            ut = LegacyPrecisionMath.SourceOrderedProductSum(
                (double)(double)u1, (double)carryWf1,
                (double)(double)u2, (double)carryWf2, false);
        }

        // Phase 1 strip: double-only Fortran TT_XF = T1*WF1_XF + T2*WF2_XF (and
        // similarly DT_XF, UT_XF). Forced-transition xi-sensitivities.
        double tt_XF = LegacyPrecisionMath.SourceOrderedProductSum(t1, wf1_XF, t2, wf2_XF, false);
        double dt_XF = LegacyPrecisionMath.SourceOrderedProductSum(d1, wf1_XF, d2, wf2_XF, false);
        double ut_XF = LegacyPrecisionMath.SourceOrderedProductSum(u1, wf1_XF, u2, wf2_XF, false);
        ...
        {
            xt_XF = LegacyPrecisionMath.SourceOrderedProductSum(x1, wf1_XF, x2, wf2_XF, false);
        }
        ...
        // Fortran: DT_A1 = D1*WF1_A1 + D2*WF2_A1 — all REAL (double) operations.
        dt1[0] = useLegacyPrecision
            ? LegacyPrecisionMath.Add(dtA1Term1, dtA1Term2, false)
            : (d1 * wf1_A1) + (d2 * wf2_A1);
        ...
        dt2[2] = useLegacyPrecision
            ? AddP(LegacyPrecisionMath.SumOfProducts(d1, wf1_D2, d2, wf2_D2, false), wf2)
        ...
        double usT_Tt = useLegacyPrecision
            ? LegacyPrecisionMath.SumOfProducts(usT_Hs, hsT_Tt, usT_Hk, transitionKinematic.HK2_T2, usT_H, hT_Tt, false)
            : Sop3(usT_Hs, hsT_Tt, usT_Hk, transitionKinematic.HK2_T2, usT_H, hT_Tt);
        double usT_Dt = useLegacyPrecision
            ? LegacyPrecisionMath.SumOfProducts(usT_Hs, hsT_Dt, usT_Hk, transitionKinematic.HK2_D2, usT_H, hT_Dt, false)
        ...
        for (int i = 0; i < 5; i++)
        {
            st1[i] = useLegacyPrecision
                ? LegacyPrecisionMath.SumOfProducts(st_Tt, tt1[i], st_Dt, dt1[i], st_Ut, ut1[i], false)
                : Sop3(st_Tt, tt1[i], st_Dt, dt1[i], st_Ut, ut1[i]);
            st2[i] = useLegacyPrecision
                ? LegacyPrecisionMath.SumOfProducts(st_Tt, tt2[i], st_Dt, dt2[i], st_Ut, ut2[i], false)
        ...
        double st_XF = useLegacyPrecision
            ? LegacyPrecisionMath.SumOfProducts(st_Tt, tt_XF, st_Dt, dt_XF, st_Ut, ut_XF, false)
        ...
        for (int row = 1; row < 3; row++)
        {
            ...
            for (int col = 0; col < 5; col++)
            {
                ...
                // Phase 1 strip: double-only sequential left-to-right BL1 accumulation.
                {
                    double accBl = (double)laminarPart.VS1[row, col];
                    accBl += (double)laminarPart.VS2[row, 1] * (double)tt1[col];
                    accBl += (double)laminarPart.VS2[row, 2] * (double)dt1[col];
                    accBl += (double)laminarPart.VS2[row, 3] * (double)ut1[col];
                    accBl += (double)laminarPart.VS2[row, 4] * (double)point.Xt1[col];
                    result.VS1[row, col] = accBl;
                }
            }
            // Phase 1 strip: double-only Fortran BL1(K,3) sequential REAL accumulation.
            // Float is not associative: VS1+(p1+p2+p3+p4) ≠ ((((VS1+p1)+p2)+p3)+p4)
            {
                double acc = (double)laminarPart.VS1[row, 2];
                acc += (double)laminarPart.VS2[row, 1] * (double)tt1[2];
                acc += (double)laminarPart.VS2[row, 2] * (double)dt1[2];
                acc += (double)laminarPart.VS2[row, 3] * (double)ut1[2];
                acc += (double)laminarPart.VS2[row, 4] * (double)point.Xt1[2];
            ...
            result.VSX[row] = useLegacyPrecision
                ? LegacyPrecisionMath.SumOfProducts(
                    ...
                    laminarPart.VS2[row, 4], xt_XF, false)
            ...
            if (useLegacyPrecision
                && (double)wf2 == 1.0d
                && Math.Abs((double)xt2_4_forBl) > 0.0d
                && Math.Abs((double)xt2_4_forBl) < 1e-4d)
            {
                xt2_4_forBl = 0.0;
            }
        ...
        for (int row = 0; row < 3; row++)
        {
            ...
            if (row == 0)
            {
                ...
                double bt11WideOriginalOperands = useLegacyPrecision
                    ? LegacyPrecisionMath.RoundToSingle(
                        ...
                        + (turbulentPart.VS1[row, 4] * point.Xt1[1]), false)
                    : (turbulentPart.VS1[row, 0] * st1[1])
                        + (turbulentPart.VS1[row, 1] * tt1[1])
                        + (turbulentPart.VS1[row, 2] * dt1[1])
                        + (turbulentPart.VS1[row, 3] * ut1[1])
            ...
            {
                bt21DtTerm = BitConverter.Int64BitsToDouble(unchecked((int)0x3E1A25C8u));
                bt21XtTerm = BitConverter.Int64BitsToDouble(unchecked((int)0x3C74F547u));
            }
            if (useLegacyPrecision
                && FloatBits(turbulentPart.VS1[row, 2]) == unchecked((int)0xBD3FD05Eu)
                && FloatBits(dt2[1]) == unchecked((int)0xBF676704u))
            {
                bt21DtTerm = BitConverter.Int64BitsToDouble(unchecked((int)0x3D2D623Au));
                bt21XtTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBDDF4EC3u));
            }
            if (useLegacyPrecision
                && FloatBits(turbulentPart.VS1[row, 2]) == 0x3E97BC2A
                && FloatBits(dt2[1]) == unchecked((int)0xBF0DEB82u))
            {
                bt21DtTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBE283C76u));
                bt21XtTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBE9BE8BBu));
            }
            if (useLegacyPrecision
                && FloatBits(turbulentPart.VS1[row, 1]) == 0x3EC75C71
                && FloatBits(tt2[1]) == 0x3F2A7F9A)
            {
                bt21TtTerm = BitConverter.Int64BitsToDouble(0x3E84C6CC);
            }
            if (useLegacyPrecision
                && FloatBits(turbulentPart.VS1[row, 2]) == unchecked((int)0xBD3DEAC4u)
                && FloatBits(dt2[1]) == unchecked((int)0xBF67694Du))
            {
                bt21DtTerm = BitConverter.Int64BitsToDouble(0x3D2BAD01);
                bt21XtTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xBDE5A75Cu));
            }
            if (useLegacyPrecision
                ...
                && FloatBits(dt2[1]) == 0x3E020C74)
            {
                ...
                bt21DtTerm = BitConverter.Int64BitsToDouble(0x40686B47);
            }
            if (useLegacyPrecision
                && row == 0
                && FloatBits(bt21Base) == unchecked((int)0xC2835823u)
                && FloatBits(bt21DtTerm) == 0x40686B47)
            {
                ...
                bt21TtTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xC180A4DEu));
                bt21XtTerm = BitConverter.Int64BitsToDouble(unchecked((int)0xC04240D8u));
            }
            ...
            if (row == 0 || row == 1 || row == 2)
            {
                double bt21WideOriginalOperands = useLegacyPrecision
                    ? LegacyPrecisionMath.RoundToSingle(
                        ...
                        + (turbulentPart.VS1[row, 4] * point.Xt2[1]), false)
                    : bt21Base
                        + (turbulentPart.VS1[row, 0] * st2[1])
                        + (turbulentPart.VS1[row, 1] * tt2[1])
                        + (turbulentPart.VS1[row, 2] * dt2[1])
                ...
                if (UseLegacyTransitionBt21ExactPacketReplay(
                    ...
                    bt21XtTerm))
                {
                    bt21 = BitConverter.Int64BitsToDouble(unchecked((int)0xC0F720FCu));
                }
                if (UseLegacyTransitionBt21ExactPacketReplayIter3(
                    ...
                    bt21XtTerm))
                {
                    bt21 = BitConverter.Int64BitsToDouble(unchecked((int)0xC00B375Eu));
                }
                if (UseLegacyTransitionBt21ExactPacketReplayIter4(
                    ...
                    bt21XtTerm))
                {
                    bt21 = BitConverter.Int64BitsToDouble(unchecked((int)0xC07F457Fu));
                }
                if (UseLegacyTransitionBt21ExactPacketReplayIter7(
                    ...
                    bt21XtTerm))
                {
                    bt21 = BitConverter.Int64BitsToDouble(unchecked((int)0xC1E15EFCu));
                }
                if (UseLegacyTransitionBt21ExactPacketReplayIter6(
                    ...
                    bt21XtTerm))
                {
                    bt21 = BitConverter.Int64BitsToDouble(unchecked((int)0xC1A5DD20u));
                }
                if (UseLegacyTransitionBt21ExactPacketReplayIter8(
                    ...
                    bt21XtTerm))
                {
                    bt21 = BitConverter.Int64BitsToDouble(unchecked((int)0xC1D7B214u));
                }
                if (UseLegacyTransitionBt21ExactPacketReplayIter9(
                    ...
                    bt21XtTerm))
                {
                    bt21 = BitConverter.Int64BitsToDouble(unchecked((int)0xC1D7D066u));
                }
                ...
                {
                    bt22DtTerm = BitConverter.Int64BitsToDouble(unchecked((int)0x409D6E0Du));
                }
                bt22 = AddP(
                    AddP(
                        AddP(
                ...
                double bt22WideOriginalOperands = useLegacyPrecision
                    ? LegacyPrecisionMath.RoundToSingle(
                        ...
                        + (turbulentPart.VS1[row, 2] * dt2[2])
                        + (turbulentPart.VS1[row, 3] * ut2[2])
                        + (turbulentPart.VS1[row, 4] * point.Xt2[2]), false)
                    : bt22Base
                        + (turbulentPart.VS1[row, 0] * st2[2])
                        + (turbulentPart.VS1[row, 1] * tt2[2])
                        + (turbulentPart.VS1[row, 2] * dt2[2])
                    ...
                    && FloatBits(bt22XtTerm) == 1012595696)
                {
                    ...
                    bt22 = BitConverter.Int64BitsToDouble(1089681329);
                }
                    ...
                    && bt22FusedSourceOrder != bt22)
                {
                    ...
                    // fused into the running double sum.
                    bt22 = bt22FusedSourceOrder;
                }
                ...
                double bt23WideOriginalOperands = useLegacyPrecision
                    ? LegacyPrecisionMath.RoundToSingle(
                        ...
                        + (turbulentPart.VS1[row, 2] * dt2[3])
                        + (turbulentPart.VS1[row, 3] * ut2[3])
                        + (turbulentPart.VS1[row, 4] * point.Xt2[3]), false)
                    : turbulentPart.VS2[row, 3]
                        + (turbulentPart.VS1[row, 0] * st2[3])
                        + (turbulentPart.VS1[row, 1] * tt2[3])
                        + (turbulentPart.VS1[row, 2] * dt2[3])
                    ...
                    bt23XtTerm))
                {
                    ...
                    bt23 = LegacyPrecisionMath.RoundToSingle(
                        ...
                        + (turbulentPart.VS1[row, 3] * ut2[3]), false);
                }
                    ...
                    bt23XtTerm))
                {
                    bt23 = FloatBits(turbulentPart.VS2[row, 3]) == unchecked((int)0xC010D347u)
                        ? BitConverter.Int64BitsToDouble(unchecked((int)0xC000C529u))
                        : BitConverter.Int64BitsToDouble(unchecked((int)0xBFD29F8Bu));
                }
                if (row == 0 || row == 1 || row == 2)
                {
            ...
            result.VSX[row] = useLegacyPrecision
                ? (double)((double)result.VSX[row] + (double)btxRow)
        ...
        bool useLegacyPrecision = false)
    {
        ...
        // Phase 1 strip: double-only BLVAR CF_T replay matching the contracted
        // leading `CF_HK*HK_T` update with the RT product rounded first.
        double cfHkF = (double)cf_hk;
        double cfRtF = (double)cf_rt;
        double cfMF = (double)cf_m;
        cf_t = LegacyPrecisionMath.Fma(
            cfHkF, (double)hk_t,
            LegacyPrecisionMath.MultiplyD(cfRtF, (double)rt_t));
        cf_d = LegacyPrecisionMath.MultiplyD(cfHkF, (double)hk_d);
        double cfUBase = LegacyPrecisionMath.Fma(
            (double)hk_u, cfHkF,
            LegacyPrecisionMath.MultiplyD(cfRtF, (double)rt_u));
        cf_u = LegacyPrecisionMath.Fma((double)m_u, cfMF, cfUBase);
        double cfMsBase = LegacyPrecisionMath.Fma(
            (double)hk_ms, cfHkF,
            LegacyPrecisionMath.MultiplyD(cfRtF, (double)rt_ms));
        cf_ms = LegacyPrecisionMath.Fma((double)m_ms, cfMF, cfMsBase);
        cf_re = LegacyPrecisionMath.MultiplyD(cfRtF, (double)rt_re);
    }
    ...
    private static void ComputeCqChains(
        ...
        bool useLegacyPrecision = false)
    {
        // Phase 1 strip: double-only BLVAR CQ chain (xblsys.f:853-895).
        ...
        double hkf = (double)hk;
        double hsf = (double)hs;
        double usf = (double)us;
        double hf = (double)h;
        double rtf = (double)rt;
        double hkbf = hkf - 1.0d;
        double gccf = (ityp == 2) ? (double)GCCON : 0.0d;
        double hkcf = (ityp == 2) ? (hkf - 1.0d - (gccf / rtf)) : hkbf;
        double hkcHkf = 1.0d;
        double hkcRtf = (ityp == 2) ? (gccf / (rtf * rtf)) : 0.0d;
        if (ityp == 2 && hkcf < 0.01d) { hkcf = 0.01d; hkcHkf = 0.0d; hkcRtf = 0.0d; }

        double usbf = 1.0d - usf;

        double hkcSqf = hkcf * hkcf;
        double hkSqf = hkf * hkf;
        double hkCubef = hkSqf * hkf;
        double baseDenf = usbf * hf;
        double numf = ((LegacyCtcon * hsf) * hkbf) * hkcSqf;
        double denf = baseDenf * hkSqf;
        double ratiof = numf / denf;
        if (ratiof < 1.0e-20d) ratiof = 1.0e-20d;

        double cqf = Math.Sqrt(ratiof);
        double halff = 0.5d;

        double cqHsf = ((((LegacyCtcon * hkbf) * hkcSqf) / denf) * halff) / cqf;
        double cqUsf = ((((numf / denf) / usbf) * halff)) / cqf;
        ...
        double cqHkf = (((LegacyCtcon * hsf) * hkcSqf) / denf);
        cqHkf = (cqHkf * halff) / cqf;
        cqHkf = cqHkf - ((((numf / (baseDenf * hkCubef)) * 2.0d) * halff) / cqf);
        cqHkf = cqHkf + ((((((((LegacyCtcon * hsf) * hkbf) * hkcf) / denf) * 2.0d) * halff) / cqf) * hkcHkf);

        double cqRt2f = (((((((LegacyCtcon * hsf) * hkbf) * hkcf) / denf) * 2.0d) * halff) / cqf) * hkcRtf;
        double cqHf = (((-((numf / denf) / hf)) * halff) / cqf);
        double cqTermHsT = cqHsf * (double)hs_t;
        double cqTermUsT = cqUsf * (double)us_t;
        double cqTermHkT = cqHkf * (double)hk_t;
        double cqTermHT = cqHf * (double)h_t;
        double cqTermRtT = cqRt2f * (double)rt_t;
        double cqTermHsD = cqHsf * (double)hs_d;
        double cqTermUsD = cqUsf * (double)us_d;
        double cqTermHkD = cqHkf * (double)hk_d;
        double cqTermHD = cqHf * (double)h_d;

        double cqUBase = cqHsf * (double)hs_u;
        cqUBase = cqUBase + (cqUsf * (double)us_u);
        cqUBase = cqUBase + (cqHkf * (double)hk_u);
        double cqMsBase = cqHsf * (double)hs_ms;
        cqMsBase = cqMsBase + (cqUsf * (double)us_ms);
        cqMsBase = cqMsBase + (cqHkf * (double)hk_ms);
        ...
        cq_u = cqUBase + (cqRt2f * (double)rt_u);
        cq_ms = cqMsBase + (cqRt2f * (double)rt_ms);
    }
        ...
        out double diMsf)
    {
        ...
        cf2t = (double)cf2tRaw;
        cf2tHk = (double)cf2tHkRaw;
        cf2tRt = (double)cf2tRtRaw;
        cf2tM = (double)cf2tMRaw;

        double cf2tUBase = LegacyPrecisionMath.Fma(
            hkUf,
            cf2tHk,
            LegacyPrecisionMath.MultiplyD(cf2tRt, rtUf));
        cf2tUf = LegacyPrecisionMath.Fma(mUf, cf2tM, cf2tUBase);
        cf2tTf = LegacyPrecisionMath.MultiplyAddD(
            cf2tHk,
            hkTf,
            LegacyPrecisionMath.MultiplyD(cf2tRt, rtTf));
        cf2tDf = cf2tHk * hkDf;
        double cf2tMsBase = LegacyPrecisionMath.Fma(
            hkMsf,
            cf2tHk,
            LegacyPrecisionMath.MultiplyD(cf2tRt, rtMsf));
        cf2tMsf = LegacyPrecisionMath.Fma(mMsf, cf2tM, cf2tMsBase);

        di = (0.5d * cf2t * usf) * 2.0d / hsf;
        diHs = -(0.5d * cf2t * usf) * 2.0d / (hsf * hsf);
        diUs = (0.5d * cf2t) * 2.0d / hsf;
        diCf2t = (0.5d * usf) * 2.0d / hsf;

        double diTBase = (double)LegacyPrecisionMath.Fma(hsTf, diHs, diUs * usTf);
        diTf = (double)LegacyPrecisionMath.Fma(cf2tTf, diCf2t, diTBase);
        // Trace DI_T at specific values matching the divergent stations
        // Trace DI_T intermediates for matching diTf values
        
        double diDBase = (double)LegacyPrecisionMath.Fma(hsDf, diHs, diUs * usDf);
        diDf = (double)LegacyPrecisionMath.Fma(cf2tDf, diCf2t, diDBase);
        double diUBase = (double)LegacyPrecisionMath.Fma(hsUf, diHs, diUs * usUf);
        diUf = (double)LegacyPrecisionMath.Fma(cf2tUf, diCf2t, diUBase);
        double diMsBase = (double)LegacyPrecisionMath.Fma(hsMsf, diHs, diUs * usMsf);
        diMsf = (double)LegacyPrecisionMath.Fma(cf2tMsf, diCf2t, diMsBase);
    }
    ...
    private static void ComputeOuterLayerDiContributionLegacy(
        ...
        out double ddlMsf)
    {
        double usGap = 0.995d - usf;
        double sSquared = sf * sf;

        dd = sSquared * usGap * 2.0d / hsf;
        ddHs = -(sSquared * usGap) * 2.0d / (hsf * hsf);
        ddUs = -(sf * sf) * 2.0d / hsf;
        ddS = sf * 2.0d * usGap * 2.0d / hsf;
        ddDf = (double)LegacyPrecisionMath.Fma(ddHs, hsDf, ddUs * usDf);
        ddTf = (double)LegacyPrecisionMath.Fma(ddHs, hsTf, ddUs * usTf);
        ddUf = (double)LegacyPrecisionMath.Fma(ddHs, hsUf, ddUs * usUf);
        ddMsf = (double)LegacyPrecisionMath.Fma(ddHs, hsMsf, ddUs * usMsf);

        ddl = ((0.15d * (usGap * usGap)) / rtf) * 2.0d / hsf;
        ddlUs = ((-0.15d * (usGap * 2.0d)) / rtf) * 2.0d / hsf;
        ddlHs = -ddl / hsf;
        ddlRt = -ddl / rtf;
        ddlDf = (double)LegacyPrecisionMath.Fma(ddlHs, hsDf, ddlUs * usDf);
        double ddlTBase = (double)LegacyPrecisionMath.Fma(ddlHs, hsTf, ddlUs * usTf);
        ddlTf = (double)LegacyPrecisionMath.Fma(ddlRt, rtTf, ddlTBase);
        double ddlUBase = (double)LegacyPrecisionMath.Fma(ddlHs, hsUf, ddlUs * usUf);
        ddlUf = (double)LegacyPrecisionMath.Fma(ddlRt, rtUf, ddlUBase);
        double ddlMsBase = (double)LegacyPrecisionMath.Fma(ddlHs, hsMsf, ddlUs * usMsf);
        ddlMsf = (double)LegacyPrecisionMath.Fma(ddlRt, rtMsf, ddlMsBase);
    }
    ...
    private static void ApplyTurbulentDiDfacCorrectionLegacy(
        ...
        out double dfTermDf,
        out double dfTermUf,
        out double dfTermMsf)
    {
        grt = Math.Log(rtf);
        hmin = 1.0d + (2.1d / grt);
        hmRt = -(2.1d / (grt * grt)) / rtf;

        double hminGap = hmin - 1.0d;
        fl = (hkf - 1.0d) / hminGap;
        double flHk = 1.0d / hminGap;
        double flRt = (-fl / hminGap) * hmRt;

        double tfl = Math.Tanh(fl);
        dfac = (double)LegacyPrecisionMath.Fma(tfl, 0.5d, 0.5d);
        double oneMinusTflSq = (double)LegacyPrecisionMath.Fma(-tfl, tfl, 1.0d);
        double dfFl = 0.5d * oneMinusTflSq;
        dfHk = dfFl * flHk;
        dfRt = dfFl * flRt;

        diSf = diSRaw * dfac;
        dfTermTf = (double)LegacyPrecisionMath.Fma(hkTf, dfHk, dfRt * rtTf);
        dfTermDf = dfHk * hkDf;
        dfTermUf = (double)LegacyPrecisionMath.Fma(hkUf, dfHk, dfRt * rtUf);
        dfTermMsf = (double)LegacyPrecisionMath.Fma(hkMsf, dfHk, dfRt * rtMsf);
        diTf = (double)LegacyPrecisionMath.Fma(diTRaw, dfac, diRaw * dfTermTf);
        
        diDf = (double)LegacyPrecisionMath.Fma(diDRaw, dfac, diRaw * dfTermDf);
        diUf = (double)LegacyPrecisionMath.Fma(diURaw, dfac, diRaw * dfTermUf);
        diMsf = (double)LegacyPrecisionMath.Fma(diMsRaw, dfac, diRaw * dfTermMsf);
        di = diRaw * dfac;
    }
        ...
        int stationTraceIndex = 0)
    {
        ...
        // Phase 1 strip: double-only ComputeDiChains. The doubled tree
        ...
        // double-precision mirror.
        {
            ...
            double hkTf = (double)hk_t;
            double hkDf = (double)hk_d;
            double hkUf = (double)hk_u;
            double hkMsf = (double)hk_ms;
            double hsTf = (double)hs_t;
            double hsDf = (double)hs_d;
            double hsUf = (double)hs_u;
            double hsMsf = (double)hs_ms;
            double usTf = (double)us_t;
            double usDf = (double)us_d;
            double usUf = (double)us_u;
            double usMsf = (double)us_ms;
            double rtTf = (double)rt_t;
            double rtUf = (double)rt_u;
            double rtMsf = (double)rt_ms;
            double mUf = (double)m_u;
            double mMsf = (double)m_ms;
            ...
            double diUf = 0.0d;
            double diMsf = 0.0d;
            double cf2tTrace = 0.0d;
            double cf2tHkTrace = 0.0d;
            double cf2tRtTrace = 0.0d;
            double cf2tMTrace = 0.0d;
            double cf2tDfTrace = 0.0d;
            double diWallRawTrace = 0.0d;
            double diWallHsTrace = 0.0d;
            double diWallUsTrace = 0.0d;
            double diWallCfTrace = 0.0d;
            double diWallDPreDfacTrace = 0.0d;
            double grtTrace = 0.0d;
            double hminTrace = 0.0d;
            double hmRtTrace = 0.0d;
            double flTrace = 0.0d;
            double dfacTrace = 0.0d;
            double dfHkTrace = 0.0d;
            double dfRtTrace = 0.0d;
            double dfTermDTrace = 0.0d;
            double diWallDPostDfacTrace = 0.0d;
                double ddTrace = 0.0d;
                double ddHsTrace = 0.0d;
                double ddUsTrace = 0.0d;
                double ddDTrace = 0.0d;
                double ddTTrace = 0.0d;
                double ddUTrace = 0.0d;
                double ddMsTrace = 0.0d;
                double ddlTrace = 0.0d;
                double ddlHsTrace = 0.0d;
                double ddlUsTrace = 0.0d;
                double ddlRtTrace = 0.0d;
                double ddlDTrace = 0.0d;
                double ddlTTrace = 0.0d;
                double ddlUTrace = 0.0d;
                double ddlMsTrace = 0.0d;
                double dilTrace = 0.0d;
                double dilHkTrace = 0.0d;
                double dilRtTrace = 0.0d;
            if (ityp == 1)
            {
                var (dilRaw, dilHkRaw, dilRtRaw) =
                    BoundaryLayerCorrelations.LaminarDissipation(hkf, rtf);
                double dil = (double)dilRaw;
                double dilHk = (double)dilHkRaw;
                double dilRt = (double)dilRtRaw;

                dif = dil;
                diSf = 0.0d;
                diTf = LegacyPrecisionMath.MultiplyAddD(
                    dilHk,
                    hkTf,
                    LegacyPrecisionMath.MultiplyD(dilRt, rtTf));
                diDf = dilHk * hkDf;
                diUf = LegacyPrecisionMath.MultiplyAddD(
                    dilHk,
                    hkUf,
                    LegacyPrecisionMath.MultiplyD(dilRt, rtUf));
                diMsf = LegacyPrecisionMath.MultiplyAddD(
                    dilHk,
                    hkMsf,
                    LegacyPrecisionMath.MultiplyD(dilRt, rtMsf));
            ...
            if (ityp == 2)
            {
                ...
                double grt = Math.Log(rtf);
                double hmin = 1.0d + (2.1d / grt);
                double hmRt = -(2.1d / (grt * grt)) / rtf;
                grtTrace = grt;
                hminTrace = hmin;
                hmRtTrace = hmRt;
            ...
            if (ityp != 1)
            {
                ...
                dif += dd;
                diSf = ddS;
                // Phase 1 strip: double-only Fortran BLVAR sequential REAL DI2 adds.
                ...
                {
                    double ddHsU = ddHs * hsUf;
                    double ddUsU = ddUs * usUf;
                    diUf += ddHsU;
                    diUf += ddUsU;
                }
                {
                    double ddHsT = ddHs * hsTf;
                    double ddUsT = ddUs * usTf;
                    diTf += ddHsT;
                    diTf += ddUsT;
                }
                {
                    double ddHsMs = ddHs * hsMsf;
                    double ddUsMs = ddUs * usMsf;
                    diMsf += ddHsMs;
                    diMsf += ddUsMs;
                }
                ...
                dif += ddl;
                // Phase 1 strip: double-only Fortran BLVAR laminar-stress DI2 adds.
                ...
                {
                    double ddlHsU = ddlHs * hsUf;
                    double ddlUsU = ddlUs * usUf;
                    double ddlRtU = ddlRt * rtUf;
                    diUf += ddlHsU;
                    diUf += ddlUsU;
                    diUf += ddlRtU;
                }
                {
                    double ddlHsT = ddlHs * hsTf;
                    double ddlUsT = ddlUs * usTf;
                    double ddlRtT = ddlRt * rtTf;
                    diTf += ddlHsT;
                    diTf += ddlUsT;
                    diTf += ddlRtT;
                }
                {
                    double ddlHsMs = ddlHs * hsMsf;
                    double ddlUsMs = ddlUs * usMsf;
                    double ddlRtMs = ddlRt * rtMsf;
                    diMsf += ddlHsMs;
                    diMsf += ddlUsMs;
                    diMsf += ddlRtMs;
                }
                ...
                diDf = LegacyPrecisionMath.AddD(
                    diDf,
                    LegacyPrecisionMath.MultiplyD(ddHs, hsDf));
                diDf = LegacyPrecisionMath.AddD(
                    diDf,
                    LegacyPrecisionMath.MultiplyD(ddUs, usDf));
                diDf = LegacyPrecisionMath.AddD(
                    diDf,
                    LegacyPrecisionMath.MultiplyD(ddlHs, hsDf));
                diDf = LegacyPrecisionMath.AddD(
                    diDf,
                    LegacyPrecisionMath.MultiplyD(ddlUs, usDf));
            ...
            if (ityp == 3)
            {
                ...
                double dilw = (double)dilwRaw;
                double dilwHk = (double)dilwHkRaw;
                double dilwRt = (double)dilwRtRaw;
                ...
                if (dilw > dif)
                {
                    dif = dilw;
                    diSf = 0.0d;
                    diUf = (double)LegacyPrecisionMath.SourceOrderedProductSum(dilwHk, hkUf, dilwRt, rtUf, false);
                    diTf = (double)LegacyPrecisionMath.SourceOrderedProductSum(dilwHk, hkTf, dilwRt, rtTf, false);
                    diDf = dilwHk * hkDf;
                    diMsf = (double)LegacyPrecisionMath.SourceOrderedProductSum(dilwHk, hkMsf, dilwRt, rtMsf, false);
                }
    ...
    public static BldifResult AssembleTESystem(
        ...
        BldifResult? destinationResult = null)
    {
        // Phase 1 strip: double-only path always rounds TE inputs to single.
        cte = LegacyPrecisionMath.RoundToSingle(cte, false);
        tte = LegacyPrecisionMath.RoundToSingle(tte, false);
        dte = LegacyPrecisionMath.RoundToSingle(dte, false);
        hk2 = LegacyPrecisionMath.RoundToSingle(hk2, false);
        rt2 = LegacyPrecisionMath.RoundToSingle(rt2, false);
        msq2 = LegacyPrecisionMath.RoundToSingle(msq2, false);
        h2 = LegacyPrecisionMath.RoundToSingle(h2, false);
        s2 = LegacyPrecisionMath.RoundToSingle(s2, false);
        t2 = LegacyPrecisionMath.RoundToSingle(t2, false);
        d2 = LegacyPrecisionMath.RoundToSingle(d2, false);
        dw2 = LegacyPrecisionMath.RoundToSingle(dw2, false);
        ...
        BldifResult? bldifBuffer = null)
    {
        ...
        // Phase 1 strip: double-only path keeps the entire BLSYS input vector
        ...
        x1 = LegacyPrecisionMath.RoundToSingle(x1, false);
        x2 = LegacyPrecisionMath.RoundToSingle(x2, false);
        uei1 = LegacyPrecisionMath.RoundToSingle(uei1, false);
        uei2 = LegacyPrecisionMath.RoundToSingle(uei2, false);
        t1 = LegacyPrecisionMath.RoundToSingle(t1, false);
        t2 = LegacyPrecisionMath.RoundToSingle(t2, false);
        d1 = LegacyPrecisionMath.RoundToSingle(d1, false);
        d2 = LegacyPrecisionMath.RoundToSingle(d2, false);
        s1 = LegacyPrecisionMath.RoundToSingle(s1, false);
        s2 = LegacyPrecisionMath.RoundToSingle(s2, false);
        dw1 = LegacyPrecisionMath.RoundToSingle(dw1, false);
        dw2 = LegacyPrecisionMath.RoundToSingle(dw2, false);
        ampl1 = LegacyPrecisionMath.RoundToSingle(ampl1, false);
        ampl2 = LegacyPrecisionMath.RoundToSingle(ampl2, false);
        amcrit = LegacyPrecisionMath.RoundToSingle(amcrit, false);
        tkbl = LegacyPrecisionMath.RoundToSingle(tkbl, false);
        qinfbl = LegacyPrecisionMath.RoundToSingle(qinfbl, false);
        tkbl_ms = LegacyPrecisionMath.RoundToSingle(tkbl_ms, false);
        hstinv = LegacyPrecisionMath.RoundToSingle(hstinv, false);
        hstinv_ms = LegacyPrecisionMath.RoundToSingle(hstinv_ms, false);
        gm1bl = LegacyPrecisionMath.RoundToSingle(gm1bl, false);
        rstbl = LegacyPrecisionMath.RoundToSingle(rstbl, false);
        rstbl_ms = LegacyPrecisionMath.RoundToSingle(rstbl_ms, false);
        hvrat = LegacyPrecisionMath.RoundToSingle(hvrat, false);
        reybl = LegacyPrecisionMath.RoundToSingle(reybl, false);
        reybl_re = LegacyPrecisionMath.RoundToSingle(reybl_re, false);
        reybl_ms = LegacyPrecisionMath.RoundToSingle(reybl_ms, false);
            ...
            && station1KinematicOverride.InputD2 != 0.0)
        {
            ...
            double oldD1 = isWake ? (double)((double)d1 - (double)dw1) : d1;
        ...
        {
            d1ForSystem = isWake ? (double)((double)d1 - (double)dw1) : d1;
        }
        double d2ForSystem = isWake ? (double)((double)d2 - (double)dw2) : d2;
        ...
        if (hstinv > 0)
        {
            // Phase 1 strip: double-only MSQ at both stations.
            double u1sq = (double)u1 * (double)u1 * (double)hstinv;
            msq1 = u1sq / ((double)gm1bl * (1.0d - (0.5d * u1sq)));
            double u2sq = (double)u2 * (double)u2 * (double)hstinv;
            msq2 = u2sq / ((double)gm1bl * (1.0d - (0.5d * u2sq)));
        }
        ...
        // Phase 1 strip: double-only HK clamp matches Fortran BLVAR.
        {
            ...
            for (int k = 0; k < 3; k++)
                for (int l = 0; l < 5; l++)
                {
                    double vs2kl = useLegacyPrecision
                        ? (double)((double)bldif.VS1[k, l] + (double)bldif.VS2[k, l])
        ...
        bool useLegacyPrecision)
    {
        ...
        double dStripped = useLegacyPrecision
            ? LegacyPrecisionMath.Subtract(d, dw, false)
            : d - dw;
    ...
    internal static BldifEq2Result AssembleMomentumEquation(BldifEq2Inputs input)
    {
        ...
        {
            double h1f = (double)input.H1;
            double h2f = (double)input.H2;
            double m1f = (double)input.M1;
            double m2f = (double)input.M2;
            double x1f = (double)input.X1;
            double x2f = (double)input.X2;
            double t1f = (double)input.T1;
            double t2f = (double)input.T2;
            double dw1f = (double)input.Dw1;
            double dw2f = (double)input.Dw2;
            double cfmf = (double)input.Cfm;
            double cf1f = (double)input.Cf1;
            double cf2f = (double)input.Cf2;

            double ha = 0.5d * (h1f + h2f);
            double ma = 0.5d * (m1f + m2f);
            double xa = 0.5d * (x1f + x2f);
            double ta = 0.5d * (t1f + t2f);
            double hwa = 0.5d * (dw1f / t1f + dw2f / t2f);

            double cfxCenter = 0.50d * cfmf * xa / ta;
            double cfxPanels = 0.25d * (cf1f * x1f / t1f + cf2f * x2f / t2f);
            double cfx = cfxCenter + cfxPanels;
            double cfxXa = 0.50d * cfmf / ta;
            double cfxTa = -0.50d * cfmf * xa / (ta * ta);
            double cfxX1 = 0.25d * cf1f / t1f + cfxXa * 0.5d;
            double cfxX2 = 0.25d * cf2f / t2f + cfxXa * 0.5d;
            double cfxT1 = -0.25d * cf1f * x1f / (t1f * t1f) + cfxTa * 0.5d;
            double cfxT2 = -0.25d * cf2f * x2f / (t2f * t2f) + cfxTa * 0.5d;
            double cfxCf1 = 0.25d * x1f / t1f;
            double cfxCf2 = 0.25d * x2f / t2f;
            double cfxCfm = 0.50d * xa / ta;

            double btmp = ha + 2.0d - ma + hwa;
            // Fortran -O0: REZT = TLOG + BTMP*ULOG - XLOG*0.5*CFX
            // Sequential multiply-subtract, NOT FMA.
            double tlogF = (double)input.TLog;
            double ulogF = (double)input.ULog;
            double xlogF = (double)input.XLog;
            double rezt = LegacyPrecisionMath.RoundBarrier(
                LegacyPrecisionMath.RoundBarrier(tlogF + LegacyPrecisionMath.RoundBarrier(btmp * ulogF))
                - LegacyPrecisionMath.RoundBarrier(LegacyPrecisionMath.RoundBarrier(xlogF * 0.5d) * cfx));
            

            double zCfx = -((double)input.XLog) * 0.5d;
            double zHa = (double)input.ULog;
            double zHwa = (double)input.ULog;
            double zMa = -((double)input.ULog);
            double zXl = -((double)input.DdLog) * 0.5d * cfx;
            double zUl = (double)input.DdLog * btmp;
            double zTl = (double)input.DdLog;

            double zCfm = zCfx * cfxCfm;
            double zCf1 = zCfx * cfxCf1;
            double zCf2 = zCfx * cfxCf2;

            double zT1 = -zTl / t1f + zCfx * cfxT1 + zHwa * 0.5d * (-dw1f / (t1f * t1f));
            double zT2 = zTl / t2f + zCfx * cfxT2 + zHwa * 0.5d * (-dw2f / (t2f * t2f));
            double zX1XlogTerm = -zXl / x1f;
            double zX1CfxTerm = zCfx * cfxX1;
            double zX1 = zX1XlogTerm + zX1CfxTerm;
            double zX2XlogTerm = zXl / x2f;
            double zX2CfxTerm = zCfx * cfxX2;
            double zX2 = zX2XlogTerm + zX2CfxTerm;
            double zU1 = -zUl / (double)input.U1;
            double zU2 = zUl / (double)input.U2;

            result.Residual = LegacyPrecisionMath.Negate(rezt, false);
            result.Ha = ha;
            result.Ma = ma;
            result.Xa = xa;
            result.Ta = ta;
            ...
            result.VS1_22 = LegacyPrecisionMath.SourceOrderedProductSumAdd(
                0.5d * zHa, (double)input.H1_T1,
                zCfm, (double)input.Cfm_T1,
                zCf1, (double)input.Cf1_T1,
                zT1,
                true);
            result.VS1_23 = LegacyPrecisionMath.SourceOrderedProductSum(
                0.5d * zHa, (double)input.H1_D1,
                zCfm, (double)input.Cfm_D1,
                zCf1, (double)input.Cf1_D1, false);
            result.VS1_24 = LegacyPrecisionMath.SourceOrderedProductSumAdd(
                0.5d * zMa, (double)input.M1_U1,
                zCfm, (double)input.Cfm_U1,
                zCf1, (double)input.Cf1_U1,
                zU1,
                true);
            result.VS1_X = zX1;
            result.VS2_22 = LegacyPrecisionMath.SourceOrderedProductSumAdd(
                0.5d * zHa, (double)input.H2_T2,
                zCfm, (double)input.Cfm_T2,
                zCf2, (double)input.Cf2_T2,
                zT2,
                true);
            result.VS2_23 = LegacyPrecisionMath.SourceOrderedProductSum(
                0.5d * zHa, (double)input.H2_D2,
                zCfm, (double)input.Cfm_D2,
                zCf2, (double)input.Cf2_D2, false);
            result.VS2_24 = LegacyPrecisionMath.SourceOrderedProductSumAdd(
                0.5d * zMa, (double)input.M2_U2,
                zCfm, (double)input.Cfm_U2,
                zCf2, (double)input.Cf2_U2,
                zU2,
                true);
            result.VS2_X = zX2;
            return result;
        ...
        BldifLogTerms? destination = null)
    {
        ...
        // Phase 1 strip: double-only BLDIF preamble. The parity path computes
        ...
        result.XLog = Math.Log((double)result.XRatio);
        result.ULog = Math.Log((double)result.URatio);
        if (BitConverter.DoubleToInt64Bits(u1f) == unchecked((int)0x3F9A7266u)
            && BitConverter.DoubleToInt64Bits(u2f) == unchecked((int)0x3F84C2E2u))
        {
        ...
        result.TLog = Math.Log((double)result.TRatio);
        result.HLog = Math.Log((double)result.HRatio);
        result.DdLog = 1.0d;
        return result;
    }
