...
public static class LinearVortexInviscidSolver
{
    ...
    // Difference from legacy: The method breaks the original routine into explicit assembly, LU factorization, basis backsolve, and trace stages, and it exposes the parity double solve path through state flags.
        ...
        double angleOfAttackRadians)
    {
        ...
        // Phase 1 strip: double-only path reproduces XFoil's single-precision LU.
        ...
        CopyMatrixToSingle(state.StreamfunctionInfluence, state.StreamfunctionInfluence, systemSize);
    ...
    // Difference from legacy: The managed port solves the two basis RHS vectors in a dedicated helper and explicitly mirrors the parity double backsolve path.
    ...
    {
        // Phase 1 strip: double-only path uses the parity double backsolve.
        // The doubled tree (auto-generated *.Double.cs twin via gen-double.py)
        ...
        var rhs0Single = XFoil.Solver.Numerics.SolverBuffers.BasisRhs0Double(systemSize);
        var rhs1Single = XFoil.Solver.Numerics.SolverBuffers.BasisRhs1Double(systemSize);
        for (int i = 0; i < systemSize; i++)
        {
            rhs0Single[i] = (double)state.BasisVortexStrength[i, 0];
            rhs1Single[i] = (double)state.BasisVortexStrength[i, 1];
        }
    ...
    // Difference from legacy: Fortran already stores the matrix in REAL form; the managed port copies the double matrix into a double workspace only when replaying the legacy kernel.
    // Decision: Keep the helper because it localizes the parity-only conversion.
    private static void CopyMatrixToSingle(double[,] source, double[,] destination, int size)
    {
        for (int row = 0; row < size; row++)
        {
            for (int col = 0; col < size; col++)
            {
                destination[row, col] = (double)source[row, col];
            }
        }
    }
    ...
    // Difference from legacy: The parity path keeps these as explicitly staged double products plus a double add/subtract instead of hiding the REAL assignment inside a wider helper.
        ...
        bool useLegacyPrecision)
    {
        ...
        // double before the add. The wide-accumulation form (double products +
        ...
        double cosaSingle = (double)cosa;
        double basisAlpha0Single = (double)basisAlpha0;
        double sinaSingle = (double)sina;
        double basisAlpha90Single = (double)basisAlpha90;
        double product1 = LegacyPrecisionMath.RoundBarrier(cosaSingle * basisAlpha0Single);
        double product2 = LegacyPrecisionMath.RoundBarrier(sinaSingle * basisAlpha90Single);
        ...
        bool useLegacyPrecision)
    {
        ...
        double sinaSingle = (double)sina;
        double basisAlpha90Single = (double)basisAlpha90;
        double product1 = LegacyPrecisionMath.RoundBarrier(cosaSingle * basisAlpha90Single);
        double product2 = LegacyPrecisionMath.RoundBarrier(sinaSingle * basisAlpha0Single);
        return LegacyPrecisionMath.RoundBarrier(product1 - product2);
    }
        ...
        bool useLegacyPanelingPrecision = false)
    {
        ...
        {
            // Keep the parity path on a double-rounded degree->radian conversion so
            // the legacy panel geometry and alpha superposition start from the same input.
            alphaRadians = LegacyPrecisionMath.RoundToSingle(alphaRadians, false);
        }
        ...
        int n)
    {
        ...
        double ag1 = useLegacyPrecision
            ? Math.Atan2((double)(-panel.YDerivative[0]), (double)(-panel.XDerivative[0]))
