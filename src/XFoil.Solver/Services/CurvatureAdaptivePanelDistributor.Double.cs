...
// Differences: The numerical flow stays aligned with PANGEN, but the managed code factors the monolithic routine into traceable helpers, uses generic numeric types, and routes parity-sensitive double contraction through explicit helper calls rather than implicit REAL temporaries.
...
/// The final node positions are produced by a Newton equidistribution loop
/// over a smoothed curvature-weighted composite spacing function; the
/// uniform arc-length seed on the over-sampled grid is only the Newton
/// *initial guess*, not the final distribution. Node ordering follows XFoil
/// convention: upper-surface TE (node 0) around LE to lower-surface TE
/// (node N-1), counterclockwise.
/// </summary>
public static class CurvatureAdaptivePanelDistributor
{
    /// <summary>
    ...
    // Ambient buffer-density bias, set by the Modern (#3) tree for
    // solution-adaptive refinement. When null (default), Distribute runs
    // classic Fortran-parity PANGEN and all downstream parity gates hold
    // bit-exactly. When set, the per-buffer-point scale multiplies the
    // normalized curvature weight before spline fit + Newton equidistribution,
    // biasing PANGEN to place more panels where the scale is higher.
    // Thread-local because the parallel polar harness runs many threads.
    [ThreadStatic] private static double[]? _ambientDensityScale;

    /// <summary>
    /// Scope object that restores the previous ambient density scale on
    /// dispose. Intended for <c>using (PushDensityScale(...))</c> blocks in
    /// the Modern tree.
    /// </summary>
    public readonly struct DensityScaleScope : IDisposable
    {
        private readonly double[]? previous;
        private readonly bool hasPrevious;

        internal DensityScaleScope(double[]? previous)
        {
            this.previous = previous;
            this.hasPrevious = true;
        }

        /// <summary>Restores the previous ambient density scale.</summary>
        public void Dispose()
        {
            if (hasPrevious)
            {
                _ambientDensityScale = previous;
            }
        }
    }

    /// <summary>
    /// Pushes an ambient per-buffer-point density scale for the current
    /// thread. Subsequent <see cref="Distribute"/> calls on this thread
    /// multiply the normalized curvature weight by the scale before Newton
    /// equidistribution, placing more panels where the scale is larger.
    /// The scale array length must match the buffer-point count of each
    /// subsequent input; mismatched lengths are ignored (distribution
    /// falls back to unscaled PANGEN).
    /// </summary>
    /// <param name="scale">Per-buffer-point multiplicative density bias.
    /// Pass <c>null</c> via clearing behavior — use the returned scope.</param>
    public static DensityScaleScope PushDensityScale(double[] scale)
    {
        if (scale is null)
        {
        ...
        var prev = _ambientDensityScale;
        _ambientDensityScale = scale;
        return new DensityScaleScope(prev);
    }

    /// <summary>
    /// Distributes panel nodes along an airfoil surface using XFoil's PANGEN algorithm:
    /// curvature-weighted Newton equidistribution with TE density control.
    /// </summary>
    ...
    {
        // Float-only after Phase 1 strip — the doubled tree (auto-generated
        // *.Double.cs twin via gen-double.py) gets the double-precision mirror.
        // The internal DistributeCore<T> generic remains for now and will be
        // unrolled into a non-generic double core in the cleanup pass.
    ...
    // Difference from legacy: The underlying algorithm is a direct port, but the managed version breaks the original routine into named phases, explicit trace hooks, and generic numeric operations so parity-sensitive double staging can be controlled precisely.
    ...
    {
        string traceScope = nameof(CurvatureAdaptivePanelDistributor);
        if (inputCount < 2)
        {
            return;
        }
        ...
        // Solution-adaptive bias (Modern #3 hook). When the ambient density
        // scale is present and sized correctly, multiply the normalized
        // curvature by it so Newton equidistribution favors regions with
        // higher scale. When the ambient is null or mismatched, this block
        // is a no-op — preserving bit-exact Fortran-parity on the #1 and
        // #2 trees.
        var ambientScale = _ambientDensityScale;
        if (ambientScale is not null && ambientScale.Length >= nb)
        {
            for (int i = 0; i < nb; i++)
            {
                w5[i] *= T.CreateChecked(ambientScale[i]);
                TracePangenCurvatureNode(traceScope, "modern_scaled", i + 1, sb[i], w5[i]);
            }
        }
        ...
        for (int iter = 0; iter < 20; iter++)
        {
            ...
            // The legacy double norm keeps the square-sum fused before sqrt.
            ...
            for (int i = 1; i < nn - 1; i++)
            {
                ...
                // The reference double build also contracts the curvature scaling
                ...
                // The legacy double build contracts this lower-diagonal term to a
            ...
            if (rtf != one)
            {
                ...
                // legacy double build only when this inner sum is fused.
            ...
            for (int i = 1; i < nn - 1; i++)
            {
                ...
                // fused multiply-add to follow the legacy double rounding path.
    ...
    // the reference REAL build rounds FP+FM to double first, then evaluates the mixed curvature
        ...
        where T : struct, IFloatingPointIeee754<T>
    {
        if (typeof(T) == typeof(double))
        {
            ...
            // Pure double arithmetic, left-to-right with operator precedence:
            ...
            double lb = double.CreateChecked(leftBase);
            double rb = double.CreateChecked(rightBase);
            double sc = double.CreateChecked(scale);
            double l1 = double.CreateChecked(left1);
            double r1 = double.CreateChecked(right1);
            double l2 = double.CreateChecked(left2);
            double r2 = double.CreateChecked(right2);
            double p1 = l1 * r1;
            double p2 = l2 * r2;
            double inner = p1 + p2;
            double scaled = sc * inner;
            double baseSum = lb + rb;
    ...
    // sized-to-max across cases. Pairs of double/double fields plus a
    // typed dispatcher mirror the PanelGeometryBuilder pattern.
    // -----------------------------------------------------------------
    [ThreadStatic] private static double[]? _sbD;   [ThreadStatic] private static double[]? _sbF;
    [ThreadStatic] private static double[]? _xbD;   [ThreadStatic] private static double[]? _xbF;
    [ThreadStatic] private static double[]? _ybD;   [ThreadStatic] private static double[]? _ybF;
    [ThreadStatic] private static double[]? _xbpD;  [ThreadStatic] private static double[]? _xbpF;
    [ThreadStatic] private static double[]? _ybpD;  [ThreadStatic] private static double[]? _ybpF;
    [ThreadStatic] private static double[]? _w5D;   [ThreadStatic] private static double[]? _w5F;
    [ThreadStatic] private static double[]? _w1D;   [ThreadStatic] private static double[]? _w1F;
    [ThreadStatic] private static double[]? _w2D;   [ThreadStatic] private static double[]? _w2F;
    [ThreadStatic] private static double[]? _w3D;   [ThreadStatic] private static double[]? _w3F;
    [ThreadStatic] private static double[]? _w6D;   [ThreadStatic] private static double[]? _w6F;
    [ThreadStatic] private static double[]? _snewD; [ThreadStatic] private static double[]? _snewF;
    [ThreadStatic] private static double[]? _ww1D;  [ThreadStatic] private static double[]? _ww1F;
    [ThreadStatic] private static double[]? _ww2D;  [ThreadStatic] private static double[]? _ww2F;
    [ThreadStatic] private static double[]? _ww3D;  [ThreadStatic] private static double[]? _ww3F;
    [ThreadStatic] private static double[]? _ww4D;  [ThreadStatic] private static double[]? _ww4F;
    [ThreadStatic] private static double[]? _sOutD; [ThreadStatic] private static double[]? _sOutF;
    [ThreadStatic] private static double[]? _xOutD; [ThreadStatic] private static double[]? _xOutF;
    [ThreadStatic] private static double[]? _yOutD; [ThreadStatic] private static double[]? _yOutF;
    [ThreadStatic] private static double[]? _newXD; [ThreadStatic] private static double[]? _newXF;
    [ThreadStatic] private static double[]? _newYD; [ThreadStatic] private static double[]? _newYF;
    [ThreadStatic] private static double[]? _newSD; [ThreadStatic] private static double[]? _newSF;
    [ThreadStatic] private static double[]? _finalArcD;  [ThreadStatic] private static double[]? _finalArcF;
    [ThreadStatic] private static double[]? _finalXpD;   [ThreadStatic] private static double[]? _finalXpF;
    [ThreadStatic] private static double[]? _finalYpD;   [ThreadStatic] private static double[]? _finalYpF;
    [ThreadStatic] private static double[]? _segLowerD;  [ThreadStatic] private static double[]? _segLowerF;
    [ThreadStatic] private static double[]? _segDiagD;   [ThreadStatic] private static double[]? _segDiagF;
    [ThreadStatic] private static double[]? _segUpperD;  [ThreadStatic] private static double[]? _segUpperF;
    [ThreadStatic] private static double[]? _segRhsD;    [ThreadStatic] private static double[]? _segRhsF;

    private static T[] Pool<T>(int n, ref double[]? dSlot, ref double[]? fSlot)
        where T : struct, IFloatingPointIeee754<T>
    {
        if (typeof(T) == typeof(double))
        {
        ...
        {
            var a = fSlot;
            if (a is null || a.Length < n) { a = new double[n]; fSlot = a; }
            else { Array.Clear(a, 0, n); }
            return (T[])(object)a;
        }
    }
