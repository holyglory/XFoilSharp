...
public static class DragCalculator
{
        ...
        bool useLegacyPrecision)
    {
        ...
        double shwake = useLegacyPrecision
            ? (double)((double)dsWake / (double)thwake)
            : dsWake / thwake;
        ...
        // All REAL (double) arithmetic including the power function.
        double fThw = (double)thwake;
        double fUew = (double)uewake;
        double fQinf2 = (double)qinf;
        double fShw = (double)shwake;
        double fBase = fUew / fQinf2;
        double fExp = 0.5d * (5.0d + fShw);
        double fPow = LegacyPrecisionMath.PowD(fBase, fExp);
        double cd = 2.0d * fThw * fPow;
        ...
        bool useLegacyPrecision)
    {
        ...
        // Phase 1 strip: double-only sequential REAL accumulation. The doubled
        // tree (auto-generated *.Double.cs twin via gen-double.py) gets the
        // double-precision mirror.
        double fSa = Math.Sin((double)alfa);
        double fCa = Math.Cos((double)alfa);
        double fCdf = 0.0d;
        double fQinf = (double)qinf;
        ...
        for (int side = 0; side < 2; side++)
        {
            ...
            for (int ibl = 2; ibl <= iblte && ibl < blState.MaxStations; ibl++)
            {
                ...
                double fDx = ((double)panel.X[iPan] - (double)panel.X[iPanPrev]) * fCa
                          + ((double)panel.Y[iPan] - (double)panel.Y[iPanPrev]) * fSa;
                double fTau = (double)blState.TAU[ibl, side];
                double fTauPrev = (double)blState.TAU[ibl - 1, side];
                fCdf = fCdf + 0.5d * (fTau + fTauPrev) * fDx * 2.0d / fQinf2;
            }
        }
