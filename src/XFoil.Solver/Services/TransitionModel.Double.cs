...
public static class TransitionModel
{
    ...
    // between calls. The parity path requires deterministic double arithmetic.
        ...
        ComputeAmplificationRate(double hk, double th, double rt, bool useLegacyPrecision = false)
    {
        ...
        double grcrit = useLegacyPrecision
            ? LegacyPrecisionMath.MultiplyAdd(0.7, LegacyPrecisionMath.Add(bb, 1.0, false), aa, false)
            : aa + (0.7 * (bb + 1.0));
        ...
        else
        {
            ...
            rnorm2Trace = rnorm2;
            // Phase 1 strip: double-only Fortran RFAC = 3.0*RNORM**2 - 2.0*RNORM**3
            // and RFAC_RN = 6.0*RNORM - 6.0*RNORM**2 — all in REAL.
            double rnormf = (double)rnorm;
            double rnorm2f = (double)rnorm2;
            double rnorm2Powf = Math.Pow(rnormf, 2.0d);
            double rnorm3Powf = Math.Pow(rnormf, 3.0d);
            double rnorm3f = rnorm2f * rnormf;
            rfacMulChainTrace = (3.0d * rnorm2f) - (2.0d * rnorm3f);
            rfacWideTrace = (double)((3.0 * (double)rnorm2f) - (2.0 * ((double)rnorm2f * (double)rnormf)));
            rfacPowTrace = (3.0d * rnorm2Powf) - (2.0d * rnorm3Powf);
            double threeRnorm2f = 3.0d * rnorm2f;
            double twoRnorm3f = 2.0d * rnorm3f;
            rfacMixedTrace = threeRnorm2f - twoRnorm3f;
            rfac = rfacMixedTrace;
            double sixRnormf = 6.0d * rnormf;
            double sixRnorm2f = 6.0d * rnorm2f;
        ...
        // Phase 1 strip: double-only Fortran DADR = 0.028*(HK-1.0) - 0.0345*EXP(...) — all in REAL.
        double hkMinusOnef = (double)hkMinusOne;
        double exf = (double)ex;
        double exTermf = 0.0345d * exf;
        double dadr = (0.028d * hkMinusOnef) - exTermf;

        // Phase 1 strip: double-only. Fortran: DADR_HK = 0.028 - 0.0345*EX_HK — all in REAL.
        ...
        double exHkf = (double)ex_hk;
        double dadr_hk = 0.028d - (0.0345d * exHkf);
        ...
        double af = LegacyPrecisionMath.Add(
            ...
            useLegacyPrecision);
        // Phase 1 strip: double-only Fortran AF_HMI = 2.7 - 11.0*HMI + 9.0*HMI**2 — all in REAL.
        double hmif = (double)hmi;
        double hmi2f = (double)hmi2;
        double afHmiBasef = 2.7d - (11.0d * hmif);
        double afHmiQuadf = 9.0d * hmi2f;
        ...
        ComputeAmplificationRateHighHk(double hk, double th, double rt, bool useLegacyPrecision = false)
    {
        ...
        else
        {
            double rnorm2 = LegacyPrecisionMath.Square(rnorm, useLegacyPrecision);
            double rnorm3 = LegacyPrecisionMath.Multiply(rnorm2, rnorm, useLegacyPrecision);
            // Phase 1 strip: double-only DAMPL2 RFAC = 3.0*RNORM**2 - 2.0*RNORM**3
            // and RFAC_RN = 6.0*RNORM - 6.0*RNORM**2 — all in REAL.
            double rnormDam2f = (double)rnorm;
            double rnorm2Dam2f = rnormDam2f * rnormDam2f;
            double rnorm3Dam2f = rnorm2Dam2f * rnormDam2f;
            rfac = (3.0d * rnorm2Dam2f) - (2.0d * rnorm3Dam2f);
            double sixRnormDam2f = 6.0d * rnormDam2f;
            double sixRnorm2Dam2f = 6.0d * rnorm2Dam2f;
        ...
        // Phase 1 strip: double-only DAMPL2 DADR / DADR_HK — same staging as DAMPL.
        double hkMinusOneDam2f = (double)hkMinusOne;
        double exDam2f = (double)ex;
        double exTermDam2f = 0.0345d * exDam2f;
        double dadr = (0.028d * hkMinusOneDam2f) - exTermDam2f;
        double exHkDam2f = (double)ex_hk;
        double dadr_hk = 0.028d - (0.0345d * exHkDam2f);
        ...
        double af = LegacyPrecisionMath.Add(
            ...
            useLegacyPrecision);
        // Phase 1 strip: double-only DAMPL2 AF_HMI = 2.7 - 11.0*HMI + 9.0*HMI**2 - 2.0*EXP(BRG).
        double hmiDam2f = (double)hmi;
        double hmi2Dam2f = (double)hmi2;
        double expBrgDam2f = (double)expBrg;
        double afHmiBaseDam2f = (double)(2.7 - (11.0 * hmiDam2f));
        double afHmiAccumDam2f = afHmiBaseDam2f + (9.0d * hmi2Dam2f);
        double af_hmi = afHmiAccumDam2f - (2.0d * expBrgDam2f);
        double af_hk = LegacyPrecisionMath.Multiply(af_hmi, hmi_hk, useLegacyPrecision);
        ...
        double acrit, bool useHighHkModel, bool useLegacyPrecision = false)
    {
        ...
        // Phase 1 strip: double-only AXSET RMS average. The live REAL build uses
        ...
        // rounded).
        double ax1f = (double)ax1;
        double ax2f = (double)ax2;
        double ax1Sqf = ax1f * ax1f;
        double ax2Sqf = ax2f * ax2f;
        double axSumf = ax1Sqf + ax2Sqf;
        ax1SqTrace = ax1Sqf;
        ax2SqTrace = ax2Sqf;
        axSumTrace = axSumf;
        double axsqf = 0.5d * axSumf;
        axsq = axsqf;
        ...
        {
            double axaf = Math.Sqrt(axsqf);
            axa = axaf;
            axa_ax1 = (0.5d * ax1f) / axaf;
            axa_ax2 = (0.5d * ax2f) / axaf;
        }
        ...
        TransitionPointResult? destinationResult = null)
    {
        ...
        for (int iter = 0; iter < MaxTransitionIterations; iter++)
        {
            ...
            double residual = useLegacyPrecision
                ...
                ? ((double)ampl2Iter - (double)ampl1) - ((double)ax.Ax * (double)dx)
        ...
        // Phase 1 strip: double-only path always rounds the final state to single.
        double finalX1 = LegacyPrecisionMath.RoundToSingle(x1, false);
        double finalX2 = LegacyPrecisionMath.RoundToSingle(x2, false);
        double finalT1 = LegacyPrecisionMath.RoundToSingle(t1, false);
        double finalT2 = LegacyPrecisionMath.RoundToSingle(t2, false);
        double finalD1 = LegacyPrecisionMath.RoundToSingle(d1, false);
        double finalD2 = LegacyPrecisionMath.RoundToSingle(d2, false);
        double finalU1 = LegacyPrecisionMath.RoundToSingle(u1, false);
        double finalU2 = LegacyPrecisionMath.RoundToSingle(u2, false);
        double finalAmpl1 = LegacyPrecisionMath.RoundToSingle(ampl1, false);
        double finalDx = LegacyPrecisionMath.Subtract(finalX2, finalX1, false);

        // Phase 1 strip: double-only path always recomputes the upstream kinematic.
        ...
        // Phase 1 strip: double-only path. Fortran TRCHEK2 uses the LAST Newton
        ...
        // both derivs=0).
        if (freeTransition && (double)finalWf2 == 1.0d)
        {
            finalWf2_X1 = 0.0;
            finalWf2_X2 = 0.0;
        }
        ...
        // Phase 1 strip: double-only path uses the in-loop carried Wf values.
        ...
        if (forcedTransition)
        {
            ...
            if (useLegacyPrecision
                && Math.Abs((double)point.Xt2[4]) > 0.0d
                && Math.Abs((double)point.Xt2[4]) < 1e-4d)
            {
        ...
        double finalTtCombo = useLegacyPrecision
            ? LegacyPrecisionMath.Add(
                LegacyPrecisionMath.Fma(
                    (double)finalAx.Ax_Hk2,
                    (double)finalTransitionKinematic.HK2_T2,
                    (double)finalAx.Ax_T2),
                finalTtComboRtTerm, false)
            : LegacyPrecisionMath.Add(
                LegacyPrecisionMath.Add(
                    finalTtComboHkTerm,
                    finalAx.Ax_T2,
        ...
        double ax_T1 = useLegacyPrecision
            ? LegacyPrecisionMath.Fma(
                (double)finalTtCombo,
                    (double)point.Tt1[1],
                    LegacyPrecisionMath.Fma(
                        (double)finalAx.Ax_Rt1,
                        (double)finalUpstreamKinematic.RT2_T2,
                        LegacyPrecisionMath.Fma(
                            (double)finalAx.Ax_Hk1,
                            (double)finalUpstreamKinematic.HK2_T2,
                            (double)finalAx.Ax_T1)))
            : LegacyPrecisionMath.Add(
                LegacyPrecisionMath.Add(
                    LegacyPrecisionMath.Add(
                        axT1HkTerm,
        ...
        double ax_D1 = useLegacyPrecision
            // Fortran: all in REAL
            ? (double)axD1HkTerm + ((double)finalAx.Ax_Hk2 * (double)finalTransitionKinematic.HK2_D2) * (double)point.Dt1[2]
        ...
        // Phase 1 strip: double-only TRCHEK2 keeps (XT_A2/Z_A2) inline on every
        ...
        double xtA2f = (double)point.Xt2[0];
        double zA2f = (double)zA2;
        double xt2OverZa2f = xtA2f / zA2f;

        point.Xt1[0] = (double)((double)point.Xt1[0] - ((xtA2f / zA2f) * (double)zA1));
        point.Xt1[1] = -((xtA2f / zA2f) * (double)zT1);
        point.Xt1[2] = -((xtA2f / zA2f) * (double)zD1);
        point.Xt1[3] = -((xtA2f / zA2f) * (double)zU1);
        point.Xt1[4] = (double)((double)point.Xt1[4] - ((xtA2f / zA2f) * (double)zX1));
        point.Xt2[1] = -((xtA2f / zA2f) * (double)zT2);
        point.Xt2[2] = -((xtA2f / zA2f) * (double)zD2);
        point.Xt2[3] = -((xtA2f / zA2f) * (double)zU2);
        point.Xt2[4] = (double)((double)point.Xt2[4] - (xt2OverZa2f * (double)zX2));
