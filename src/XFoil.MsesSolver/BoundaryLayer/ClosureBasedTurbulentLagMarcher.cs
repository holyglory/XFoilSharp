...
///                                      is Phase 2e scope)
///   dCτ/dξ = (K2/δ)·(Cτ_eq − Cτ)     (MsesClosureRelations.ComputeCTauLagRhs)
///
/// Cf now comes from <see cref="MsesClosureRelations.ComputeCfTurbulent"/>
/// as before, but CD is computed via
/// <see cref="MsesClosureRelations.ComputeCDTurbulent"/> using the
...
public static class ClosureBasedTurbulentLagMarcher
{
        ...
        double machNumberEdge = 0.0)
    {
        ...
        for (int i = 1; i < n; i++)
        {
            ...
            double HkMid = MsesClosureRelations.ComputeHk(HMid, machNumberEdge);
        ...
        double Ue, double dUeDx, double nu, double Me)
    {
        ...
        double cTauEq = MsesClosureRelations.ComputeCTauEquilibrium(Hk, ReTheta, Me);
        ...
        // Cτ lag.
        double dCTauDx = MsesClosureRelations.ComputeCTauLagRhs(cTau, cTauEq, theta, Hk);
