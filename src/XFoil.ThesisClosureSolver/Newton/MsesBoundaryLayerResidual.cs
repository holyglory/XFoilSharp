...
/// <see cref="XFoil.ThesisClosureSolver.BoundaryLayer.ThesisExactTurbulentMarcher"/>.
...
/// <see cref="ThesisClosureGlobalResidual"/> in a later iteration (P5.3).
/// For now they live as standalone, unit-testable functions.
/// </summary>
public static class ThesisClosureBoundaryLayerResidual
{
    /// <summary>
        ...
        bool isWake = false)
    {
        ...
        double hkAvg = ThesisClosureRelations.ComputeHk(hAvg, me);
        double reThetaAvg = 0.5 * (uePrev + ue) * thetaAvg
                          / System.Math.Max(nu, 1e-18);
        double cfAvg = isWake
            ? 0.0
            : ThesisClosureRelations.ComputeCfTurbulent(hkAvg, reThetaAvg, me);
        ...
        bool isWake = false)
    {
        ...
        double hkPrev = ThesisClosureRelations.ComputeHk(hPrev, me);
        double hStar = ThesisClosureRelations.ComputeHStarTurbulent(hk, reTheta, me);
        double hStarPrev = ThesisClosureRelations.ComputeHStarTurbulent(
            hkPrev, reThetaPrev, me);
        double cf = isWake
            ? 0.0
            : ThesisClosureRelations.ComputeCfTurbulent(hk, reTheta, me);
        ...
        double me = 0.0)
    {
        ...
        double hkMid = ThesisClosureRelations.ComputeHk(hMid, me);
        double reTMid = ueMid * thetaMid / System.Math.Max(nu, 1e-18);
        double cTauEq = ThesisClosureRelations.ComputeCTauEquilibrium(
            hkMid, reTMid, me);
