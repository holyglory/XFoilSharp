...
/// Newton system. Uses <see cref="ThesisClosureGlobalStateSided"/> (per-side
...
/// R5.8 gate.
/// </summary>
public sealed class ThesisClosureGlobalResidualSided
{
    private readonly ThesisClosureGlobalStateSided _layout;
    ...
    public ThesisClosureGlobalResidualSided(
        ThesisClosureGlobalStateSided layout,
        ...
        WakeDiscretization.WakePanels? wake = null)
    {
        ...
        {
            _aWakeSigmaNormal = ThesisClosurePanelSolver.BuildWakeSourceInfluenceMatrix(
                pg, wake.Value, normal: true);
            _aWakeSigmaTangent = ThesisClosurePanelSolver.BuildWakeSourceInfluenceMatrix(
    ...
    private void ComputeWakeBlResiduals(
        ThesisClosureGlobalStateSided.SidedState u, double[] r)
    {
        ...
        var te = ThesisClosureBoundaryLayerResidual.TEMergeResiduals(
        ...
        for (int k = 1; k < nw; k++)
        {
            ...
            r[_layout.WakeThetaOffset + k] = ThesisClosureBoundaryLayerResidual.MomentumResidual(
                thetaPrev, thetaCur, hPrev, hCur, ueMid, ueMid, dx,
                _kinematicViscosity, _machEdge, isWake: true);
            r[_layout.WakeDstarOffset + k] = ThesisClosureBoundaryLayerResidual.ShapeParamResidual(
                thetaPrev, thetaCur, hPrev, hCur, u.WakeCTau[k], ueMid, ueMid, dx,
                _kinematicViscosity, _machEdge, isWake: true);
            r[_layout.WakeCTauOffset + k] = ThesisClosureBoundaryLayerResidual.LagResidual(
        ...
        int dStarOffset, int thetaOffset, int cTauOffset)
    {
        ...
        for (int k = 1; k < ns; k++)
        {
            ...
            r[thetaOffset + k] = ThesisClosureBoundaryLayerResidual.MomentumResidual(
                thetaPrev, thetaCur, hPrev, hCur, uePrev, ueCur, dx,
                _kinematicViscosity, _machEdge);
            r[dStarOffset + k] = ThesisClosureBoundaryLayerResidual.ShapeParamResidual(
                thetaPrev, thetaCur, hPrev, hCur, cTau[k], uePrev, ueCur, dx,
                _kinematicViscosity, _machEdge);
            r[cTauOffset + k] = ThesisClosureBoundaryLayerResidual.LagResidual(
    ...
    private void ComputeSigmaAirfoilResiduals(
        ThesisClosureGlobalStateSided.SidedState u, double[] ueMid, double[] r)
    {
        int n = _pg.PanelCount;
