...
public static class LaminarTransitionMarcher
{
        ...
        bool useThesisExactLaminar = false)
    {
        ...
        for (int i = 1; i < n; i++)
        {
            ...
            double hkMid = 0.5 * (
                XFoil.ThesisClosureSolver.Closure.ThesisClosureRelations.ComputeHk(h[i - 1], machNumberEdge)
                + XFoil.ThesisClosureSolver.Closure.ThesisClosureRelations.ComputeHk(h[i], machNumberEdge));
