...
public class ThesisClosureAnalysisService : IAirfoilAnalysisService
{
    ...
    public ThesisClosureAnalysisService(
        ...
        double xiOffset)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            double hk = ThesisClosureRelations.ComputeHk(h, 0.0);
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (i > 0 && theta > 1e-10 && reTheta > 10.0)
            {
                ...
                if (transitionIdx < 0 || i < transitionIdx)
                {
                    ...
                    cf = ThesisClosureRelations.ComputeCfLaminar(hkLam, reTheta);
                }
                else
                {
                    cf = ThesisClosureRelations.ComputeCfTurbulent(hk, reTheta, 0.0);
                }
            }
            profiles[i] = new BoundaryLayerProfile
            {
