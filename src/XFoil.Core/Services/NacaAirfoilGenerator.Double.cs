...
namespace XFoil.Core.Double.Services;
...
public class NacaAirfoilGenerator
{
    ...
    {
        string precision = typeof(T) == typeof(double) ? "Single" : "Double";
    ...
    // Difference from legacy: The method factors the law into a reusable helper and adds an explicit double replay branch so parity mode preserves `powf`-style staging and multiply order.
        ...
        where T : struct, IFloatingPointIeee754<T>
    {
        ...
        {
            double fractionSingle = double.CreateChecked(fraction);
            double oneMinusFractionSingle = double.CreateChecked(oneMinusFraction);
            double exponentSingle = double.CreateChecked(exponent);
            double exponentPlusOneSingle = double.CreateChecked(exponentPlusOne);
            ...
            double sqrtOneMinusFractionSingle = Math.Sqrt(oneMinusFractionSingle);
            double powerAnSingle = Math.Pow(oneMinusFractionSingle, exponentSingle);
            double powerAnpSingle = Math.Pow(oneMinusFractionSingle, exponentPlusOneSingle);
            ...
            double leadingTermSingle = (exponentPlusOneSingle * fractionSingle) * powerAnSingle;
            double xSingle = (1.0d - leadingTermSingle) - powerAnpSingle;
        ...
        where T : struct, IFloatingPointIeee754<T>
    {
        if (typeof(T) == typeof(double))
        {
            double xSingle = double.CreateChecked(x);
            double x2Single = xSingle * xSingle;
            double x3Single = x2Single * xSingle;
            ...
            double x4Single = x2Single * x2Single;
