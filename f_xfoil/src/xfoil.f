...
C
      DO I = 1, NB
        CALL TRACE_BUFFER_NODE('PANGEN', I, XB(I), YB(I), SB(I))
      ENDDO
C
...
C---- set up curvature array
      DO I = 1, NB
        ...
        CALL TRACE_PANGEN_CURVATURE_NODE('PANGEN', 'raw',
     &       I, SB(I), W5(I))
      ENDDO
C
...
C---- set average curvature over 2*NK+1 points within Rcurv of LE point
      ...
      DO K = -NK, NK
        ...
        CVSUM = CVSUM + CVK
        CALL TRACE_PANGEN_LE_SAMPLE('PANGEN', 'buffer', K, FRAC,
     &       SBK, CVK, CVSUM)
      ENDDO
...
C---- dummy curvature for sharp LE
      ...
      CALL TRACE_PANGEN_LEFIND('PANGEN', 'buffer', SBLE, XBLE, YBLE,
     &     XBTE, YBTE, CVLE, CVAVG, IBLE)
C
...
C---- normalize curvature array
      DO I=1, NB
        W5(I) = W5(I) / CVMAX
        CALL TRACE_PANGEN_CURVATURE_NODE('PANGEN', 'normalized',
     &       I, SB(I), W5(I))
      ENDDO
C
...
C
      ...
      DO I=1, NN
        CALL TRACE_PANGEN_SNEW_NODE('PANGEN', 'initial', 0, I,
     &       SNEW(I))
      ENDDO
C
...
C         (1 + C*curv)*deltaS is equal on both sides of node i
          W4(I) = -REZ
C
          CALL TRACE_PANGEN_NEWTON_STATE('PANGEN', ITER, I, SNEW(I),
     &         DSM, DSP, CV1, CV2, CV3, CVS1, CVS2, CVS3,
     &         CAVM, CAVM_S1, CAVM_S2, CAVP, CAVP_S2, CAVP_S3,
     &         FM, FP, REZ, W1(I), W2(I), W3(I), W4(I))
C
          CV1 = CV2
          CV2 = CV3
          CVS1 = CVS2
...
C------ fix sharp LE point
        ...
        ENDIF
C
C------ trace assembled Newton system before solve
        DO I=1, NN
...
C------ trace solved arc-length deltas
        DO I=1, NN
          CALL TRACE_PANGEN_NEWTON_DELTA('PANGEN', ITER, I, W4(I))
        ENDDO
C
C------ find under-relaxation factor to keep nodes from changing order
        ...
        DO I=1, NN-1
          ...
          CALL TRACE_PANGEN_RELAXATION_STEP('PANGEN', ITER, I, DS,
     &         DDS, DSRAT, RLXOLD, RLX, DMAXOLD, DMAX)
        ENDDO
C
C------ update node position
        ...
        CALL TRACE_PANGEN_ITERATION('PANGEN', ITER, DMAX, RLX)
        DO I=1, NN
          CALL TRACE_PANGEN_SNEW_NODE('PANGEN', 'updated', ITER, I,
     &         SNEW(I))
        ENDDO
C
...
C
      ...
      CALL TRACE_PANGEN_LEFIND('PANGEN', 'final', SLE, XLE, YLE,
     &     XTE, YTE, 0.0, 0.0, 0)
C
