...
C
      CALL TRACE_NACA4_CONFIG('NacaAirfoilGenerator.Generate4DigitCore',
     & IDES, 2*NSIDE-1, M, P, T)
C
      ANP = AN + 1.0
      DO 10 I=1, NSIDE
        ...
        IF(I.EQ.NSIDE) THEN
         ...
         XX(I) = 1.0
        ELSE
         SQRTONEMF = SQRT(ONEMF)
         XPOWAN = ONEMF**AN
         XPOWANP = ONEMF**ANP
         XTERM1 = ANP*FRAC*XPOWAN
         XTERM2 = XPOWANP
         XX(I) = 1.0 - XTERM1 - XTERM2
        ENDIF
        X2TRACE = XX(I)**2
        X3TRACE = XX(I)**3
        X4TRACE = XX(I)**4
     ...
     &          - 0.10150*XX(I)**4) * T / 0.20
        YTSQRT = SQRT(XX(I))
        YTTERM0 = 0.29690*YTSQRT
        YTTERM1 = -0.12600*XX(I)
        YTTERM2 = -0.35160*XX(I)**2
        YTTERM3 = 0.28430*XX(I)**3
        YTTERM4 = -0.10150*XX(I)**4
        YTPART1 = YTTERM0 + YTTERM1
        YTPART2 = YTPART1 + YTTERM2
        YTPART3 = YTPART2 + YTTERM3
        YTPOLY = YTPART3 + YTTERM4
        IF(XX(I).LT.P) THEN
         CAMBERBRANCH = 1
         YC(I) = M/P**2 * (2.0*P*XX(I) - XX(I)**2)
         DYCDX = 2.0*M/P**2 * (P - XX(I))
        ELSE
         CAMBERBRANCH = 2
         YC(I) = M/(1.0-P)**2 * ((1.0-2.0*P) + 2.0*P*XX(I)-XX(I)**2)
         DYCDX = 2.0*M/(1.0-P)**2 * (P - XX(I))
        ENDIF
        CALL TRACE_NACA4_NODE('NacaAirfoilGenerator.Generate4DigitCore',
     &   I, FRAC, ONEMF, USEDTE, SQRTONEMF, XPOWAN, XPOWANP,
     &   XTERM1, XTERM2, XX(I), X2TRACE, X3TRACE, X4TRACE,
     &   YTSQRT, YTSCALE,
     &   YTTERM0, YTTERM1, YTTERM2, YTTERM3, YTTERM4,
     &   YTPART1, YTPART2, YTPART3, YTPOLY, YT(I),
     &   CAMBERBRANCH, YC(I), DYCDX)
   10 CONTINUE
C
      IB = 0
      DO 20 I=NSIDE, 1, -1
        ...
        XB(IB) = XX(I)
        YB(IB) = YC(I) + YT(I)
        CALL TRACE_NACA_GEOMETRY_POINT(
     &   'NacaAirfoilGenerator.Generate4DigitCore', IB, XB(IB), YB(IB))
   20 CONTINUE
      DO 30 I=2, NSIDE
        IB = IB + 1
        XB(IB) = XX(I)
